...
(function () {
    ...
    function _e(s) { if (typeof esc === "function")
        return esc(s); return String(s == null ? "" : s).replace(/[&<>"']/g, function (c) { return { "&": "&amp;", "<": "&lt;", ">": "&gt;", "\"": "&quot;", "'": "&#39;" }[c]; }); }
    function _isAdmin() {
        return !!(window._currentUser && window._currentUser.role === "admin");
    }
    function _t(obj) {
        // Re-use the shared connectors_common helper if loaded
        if (typeof window._connT === "function")
            return window._connT(obj);
        if (!obj)
            return "";
    ...
    function _renderFilterBar(g, total) {
    ...
    window._connSetFilter = function (mod) {
        _connFilter = mod || "all";
    ...
    function _renderInstanceList(conn) {
    ...
    window._renderConnectors = function (c) {
        if (!_isAdmin()) {
        ...
        _loadAggregate().then(function (data) {
            ...
            if (!list.length) {
                ...
                if (_connFilter !== "all" && g.mods.indexOf(_connFilter) < 0)
                    _connFilter = "all";
    ...
    window._connOpenConfig = function (id) {
        ...
        if (pre.graph_permissions && pre.graph_permissions.length) {
            ...
            prereqHtml += pre.graph_permissions.map(function (p) {
            ...
            buttons: [
                { id: "cancel", label: t("pilot.action.cancel") },
                {
                    ...
                    result: function () {
                        ...
                        (schema.fields || []).forEach(function (f) {
                            if (f.secret && payload[f.id] === "configured")
                                delete payload[f.id];
                        });
    ...
    async function _putAggregator(id, payload) {
    ...
    window._connRunTest = function (id) {
        var out = document.querySelector('[data-conn-result="' + id + '"]');
        if (out) {
            out.style.color = "var(--ct-ink-2)";
            out.textContent = t("pilot.connectors.testing");
        }
        fetch("api/admin/connectors/" + encodeURIComponent(id) + "/test", {
            ...
            if (out) {
                out.style.color = "var(--danger,var(--ct-critical))";
                out.textContent = t("pilot.connectors.network_error");
            }
        });
    };
    window._connRunNow = function (id) {
        var out = document.querySelector('[data-conn-result="' + id + '"]');
        if (out) {
            out.style.color = "var(--ct-ink-2)";
            out.textContent = t("pilot.connectors.computing");
        }
        fetch("api/admin/connectors/" + encodeURIComponent(id) + "/run", {
            ...
            if (res.results) {
                summary = Object.keys(res.results).map(function (k) {
                    ...
                    if (r.ok === false) {
                        hasError = true;
                        return k + ": " + (r.skipped || t("pilot.connectors.not_configured_lc"));
                    }
    ...
    window._connDeleteConfig = function (id) {
        var entry = (_aggCache && _aggCache.connectors || []).find(function (x) { return x.id === id; });
    ...
    window._connOpenInstance = function (typeId, ownerModule, instanceId) {
        ...
        if (instance.project_id)
            body += ' · ' + _e(t("pilot.connectors.project")) + ' <code>' + _e(instance.project_id) + '</code>';
        body += '</div>';
        if (typeof window._connRenderForm === "function") {
    ...
    async function _putInstance(typeId, ownerModule, instanceId, payload) {
    ...
    window._connNewInstance = function (typeId) {
        var entry = (_aggCache && _aggCache.connectors || []).find(function (x) { return x.id === typeId; });
        if (!entry)
            return;
        var schema = entry.schema || {};
        ...
        if (consumers.length === 1)
            body += ' ' + _e(t("pilot.connectors.on")) + ' <b>' + _e(_consumerLabel(consumers)) + '</b>';
        body += '</div>';
        body += modulePicker;
            ...
            buttons: [
                { id: "cancel", label: t("pilot.action.cancel") },
                {
                    ...
                    result: function () {
                        ...
                        var labelEl = root.querySelector('[data-conn-new-label]');
                        payload.label = labelEl ? labelEl.value : "";
                        var modEl = root.querySelector('[data-conn-new-module]');
    ...
    async function _postCreateInstance(typeId, body) {
    ...
    window._connTestInstance = function (typeId, ownerModule, instanceId) {
        var out = document.querySelector('[data-conn-result="' + typeId + '__' + instanceId + '"]');
        if (out) {
            out.style.color = "var(--ct-ink-2)";
            out.textContent = t("pilot.connectors.test_ellipsis");
        }
        var url = "api/admin/connectors/" + encodeURIComponent(typeId)
            + "/instances/" + encodeURIComponent(ownerModule)
