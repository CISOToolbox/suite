...
(function () {
    ...
    var _dashData = null;
    var _measures = [];
    var _groups = []; // FEAT-11 meta-measures
    var _evidences = []; // FEAT-08 consolidated evidence registry
    var _evidenceSyncStatus = "";
    var _evidenceFilter = { module: "", search: "" };
    var _users = [];
    var _modules = [];
    var _projects = [];
    var _editingProject = null;
    var BASE = "api";
    ...
    function _pilotTone(v) {
        return _PILOT_TONES[(v || "").toString()] || "neutral";
    }
    function _pilotModuleRef(mod) {
        var m = (mod || "").toString();
    ...
    window._openNotifPrefs = function () {
        ...
        window.ct_notifprefs.open({
            ...
            savePrefs: function (prefs) { return _fetch("/me/notification-prefs", { method: "PUT", body: prefs }); },
    ...
    window.selectPanel = function (id) {
        ...
        if (id === "dashboard") {
            _refreshTimer = setInterval(function () {
                _loadDashboard().then(function () { if (_panel === "dashboard")
                    _renderPanel(); });
            }, 30000);
        }
        // Sync measures on entry into the measures panel so freshly created module measures appear immediately
        if (id === "measures") {
            _syncMeasuresBackground().then(function () { if (_panel === "measures")
                _renderPanel(); });
        }
        if (id === "evidences") {
            _syncEvidencesBackground().then(function () { if (_panel === "evidences")
                _renderPanel(); });
        }
        if (id === "backups") {
    ...
    window.renderAll = function () { _renderPanel(); };
    ...
    if (_measureView !== "list" && _measureView !== "kanban")
        _measureView = "list";
    function _renderMeasures(c) {
        ...
        h += '</div>';
        if (_syncStatus)
            h += '<div style="font-size:var(--ct-text-label);color:var(--ct-ink-2);margin:var(--ct-s2) 0 var(--ct-s3)">' + esc(_syncStatus) + '</div>';
        if (!_measures.length) {
        ...
        var filtered = _measures.filter(function (m) {
            if (_measureFilter.module && m.module !== _measureFilter.module)
                return false;
            if (_measureView === "list" && _measureFilter.status && m.status !== _measureFilter.status)
                return false;
            ...
            if (_measureView === "list" && _measureFilter.orphan && assigned[m.id])
                return false;
        ...
        if (_measureView === "list") {
            ...
            h += _renderMeasuresList(_groupRows(groupsShown).concat(filtered), measureProject, today);
            c.innerHTML = h;
            ct_bulkbar.attach({
    ...
    function _renderMeasuresList(filtered, measureProject, today) {
            ...
            columns: [
                ...
                { key: "source_id", label: t("pilot.col.ref"), width: "120px",
                    render: function (m) {
                        var r = m;
                        if (r.__group_id)
                            return '<span class="ct-badge" data-size="sm" data-tone="info" title="' + r.__member_count + ' ' + esc(t("pilot.groups.members_word")) + '">' + _icon("link", 10) + ' ' + esc(r.__ref || String(r.__member_count)) + '</span>';
                ...
                { key: "title", label: t("pilot.col.measure"),
                    render: function (m) {
                        ...
                        var h2 = esc(m.title || "");
                        if (r.__diverged)
                            h2 += ' <span class="ct-badge" data-size="sm" data-tone="medium" title="' + esc(t("pilot.groups.diverged_hint")) + '">' + esc(t("pilot.groups.diverged")) + '</span>';
                        return h2;
                    } },
                ...
                { key: "project", label: t("pilot.col.project"), width: "140px",
                    render: function (m) {
                        ...
                        var p = measureProject[m.id];
                        if (p)
                            return '<span class="ct-badge" data-tone="info" title="'
    ...
    function _wireKanbanDnD() {
        var cards = document.querySelectorAll(".pilot-kanban-card[draggable=\"true\"]");
        ...
        var cols = document.querySelectorAll(".pilot-kanban-col-body[data-status]");
        cols.forEach(function (col) {
            ...
            col.addEventListener("drop", function (e) {
                ...
                if (id.indexOf("group:") === 0)
                    _moveGroupStatus(id.slice(6), newStatus);
                else
    ...
    function _moveMeasureStatus(measureId, newStatus) {
        var m = _measures.find(function (x) { return x.id === measureId; });
        if (!m)
            return;
        if (m.status === newStatus)
            return; // no-op drop on the same column
    ...
    function _moveGroupStatus(groupId, newStatus) {
    ...
    window._setMeasureView = function (view) {
        if (view !== "list" && view !== "kanban")
            return;
        _measureView = view;
        try {
            localStorage.setItem("pilot_measures_view", view);
        }
        catch (e) { }
        _renderPanel();
    };
    ...
    // ═══════════════════════════════════════════════════════════════
    function _groupDiverged(g, mm) {
        return (mm.status || "") !== (g.status || "")
            || (mm.due_date || "") !== (g.due_date || "")
            || (mm.assignee || "") !== (g.responsible || "");
    }
    function _groupModulesBadge(g) {
        var mods = [];
        (g.members || []).forEach(function (mm) { if (mods.indexOf(mm.module) < 0)
            mods.push(mm.module); });
        return mods.join(" + ");
    }
    ...
    function _renderGroupCard(g, today) {
        ...
        h += '<div class="pilot-kanban-card-foot">';
        if (g.due_date)
            h += '<span class="ct-text-meta">' + esc(g.due_date) + '</span>';
        if (g.responsible)
            h += '<span class="ct-text-meta">' + esc(g.responsible) + '</span>';
        h += '</div></div>';
        return h;
    }
    window._openGroupMemberModule = function (gid, mid) {
        var g = _groups.find(function (x) { return x.id === gid; });
        var mm = g && (g.members || []).find(function (x) { return x.id === mid; });
        if (mm)
            window.open(_moduleMeasureUrl(mm), "_blank");
    };
    window._openGroupRow = function (gid) {
        var g0 = _groups.find(function (x) { return x.id === gid; });
        if (!g0 || !window.ct_measure_modal)
            return;
        var g = g0;
        var diverged = (g.members || []).some(function (mm) { return _groupDiverged(g, mm); });
        ...
        (g.members || []).forEach(function (mm) {
            ...
            if (d)
                mh += '<span title="' + esc(t("pilot.groups.diverged_hint")) + '">\u26A0</span>';
        ...
        if (diverged)
            extra.unshift({ id: "resync", label: t("pilot.groups.resync"),
                result: function () { return { __resync: true }; } });
        ct_measure_modal.open({ title: g.title, statut: g.status, responsable: g.responsible, echeance: g.due_date }, {
            ...
            if (!result)
                return;
            if (result.__resync) {
                window._resyncGroup(g.id);
                return;
            }
            if (result.__dissolve) {
                if (!confirm(t("pilot.groups.dissolve_confirm")))
                    return;
    ...
    window._detachGroupMember = function (gid, mid) {
    ...
    window._resyncGroup = function (gid) {
    ...
    function _aiRenderSuggestedGroups() {
        ...
        valid.forEach(function (g, i) {
            ...
            h += '</div>';
            if (g.reason)
                h += '<div class="ai-card-reason ct-mt-1">' + esc(g.reason) + '</div>';
            h += '</div></div></div>';
        });
        p.body.innerHTML = h;
    ...
    window._aiCreateSuggestedGroups = function () {
        var checked = [];
        document.querySelectorAll(".ai-group-cb:checked").forEach(function (cb) {
        ...
        function next() {
            ...
            if (!g) {
                ...
                if (propErrors)
                    msg += " — " + t("pilot.groups.saved_with_errors", { n: propErrors });
    ...
    window._openMeasureRow = function (row) {
        var gid = row.__group_id;
        if (gid) {
            window._openGroupRow(gid);
            return;
        }
        if (row)
            window._openMeasure(row.module, row.source_id);
    };
    window._bulkPilotMeasuresDelete = function (scope) {
        var ids = Array.from(ct_bulkbar.getSelection(scope)).filter(function (id) { return id.indexOf("grp:") !== 0; });
        if (!ids.length)
            return;
    ...
    function _showMeasureModal(em) {
        ...
        ct_measure_modal.open(em, {
            ...
            if (!result)
                return;
            if (result.__deleted)
                return; // onDelete already invoked
            if (result.__open_module) {
                window.open(_moduleMeasureUrl(em), "_blank");
                return;
            }
            _saveMeasureEditFromValues(em, result);
        });
    }
    function _saveMeasureEditFromValues(m, values) {
        if (!m || _savingMeasure)
            return;
        var patch = {};
        if (values.title != null && values.title !== (m.title || ""))
            patch.title = values.title;
        if (values.description != null && values.description !== (m.description || ""))
            patch.description = values.description;
        if (values.status != null && values.status !== m.status)
            patch.status = values.status;
        if (values.assignee != null && values.assignee !== (m.assignee || ""))
            patch.assignee = values.assignee;
        if (values.due_date != null && values.due_date !== (m.due_date || ""))
            patch.due_date = values.due_date;
        ...
        if (!Object.keys(patch).length && !projectChanged) {
            showStatus(t("pilot.common.no_changes"));
            return;
        }
        _savingMeasure = true;
        function _applyProjectDiff() {
            ...
            var p1 = oldPid
                ? _fetch("/projects/" + encodeURIComponent(oldPid) + "/measures/" + encodeURIComponent(m.id), { method: "DELETE" }).catch(function () { })
                : Promise.resolve();
            // 2) link to new project if any
            return p1.then(function () {
                if (!newPid)
                    return;
                return _fetch("/projects/" + encodeURIComponent(newPid) + "/measures", { method: "POST", body: { measure_ids: [m.id] } });
            });
        }
    ...
    function _deleteMeasureEdit(em) {
        if (!em)
            return;
    ...
    window._filterMeasures = function () {
        ...
        var modSel = document.getElementById("filter-module");
        var projSel = document.getElementById("filter-project");
        var stSel = document.getElementById("filter-status");
        var odCb = document.getElementById("filter-overdue");
        var orphanCb = document.getElementById("filter-orphan");
        ...
        _renderPanel();
        if (hadFocus) {
            var newSearchEl = document.getElementById("filter-search");
            if (newSearchEl) {
                newSearchEl.focus();
                try {
    ...
    var _editingMeasure = null;
    var _savingMeasure = false;
    window._openMeasure = function (modId, sourceId) {
    ...
    function _syncMeasuresBackground() {
        ...
        return _fetch("/measures/sync", { method: "POST" }).then(function (report) {
            ...
            for (var mod in report) {
                var r = report[mod];
                if (r.error)
                    parts.push(t("pilot.sync.module_error", { mod: mod }));
                else
                    parts.push(t("pilot.sync.module_measures", { mod: mod, n: r.added + r.updated }));
            }
    ...
    // PROJETS
    // ═══════════════════════════════════════════════════════════════
    function _renderProjects(c) {
        if (_editingProject) {
            _renderProjectEdit(c);
            return;
        }
        ...
        _projects.forEach(function (p) {
            ...
            if (prioCls)
                h += '<span class="ct-badge" data-tone="' + _pilotTone(prioCls) + '" style="margin-right:var(--ct-s1)">' + tEsc("pilot.project.priority." + p.priority) + '</span>';
            ...
            if (p.responsible || p.due_date) {
                ...
                if (p.responsible && p.due_date)
                    h += ' &mdash; ';
                if (p.due_date)
                    h += t("pilot.projects.due_prefix") + esc(p.due_date);
                h += '</div>';
            }
    ...
    function _renderProjectEdit(c) {
        ...
        setTimeout(function () {
            if (typeof ct_userpicker === "undefined" || !ct_userpicker.mount)
                return;
            ct_userpicker.mount({
    ...
    window._editProject = function (id) {
    ...
    window._saveProject = function () {
        var nameVal = (document.getElementById("pj-name").value || "").trim();
        if (!nameVal) {
            showStatus(t("pilot.projects.name_required"), true);
            var nm = document.getElementById("pj-name");
            if (nm) {
                nm.focus();
                nm.style.borderColor = "var(--ct-critical)";
            }
            return;
        }
        var data = {
        ...
        var cascadeEl = document.getElementById("pj-cascade");
        if (cascadeEl && cascadeEl.checked)
            data.cascade = true;
        var promise;
    ...
    window._deleteProject = function (id) {
        if (!confirm(t("pilot.projects.delete_confirm")))
            return;
    ...
    window._removeMeasure = function (projectId, measureId) {
    ...
    window._addSelectedMeasures = function () {
        if (!_editingProject || !_editingProject.id)
            return;
        var cbs = document.querySelectorAll(".pj-measure-cb:checked");
        if (!cbs.length) {
            showStatus(t("pilot.measures.none_selected"));
            return;
        }
        var ids = [];
        var lines = [];
        ...
        _fetch("/projects/" + _editingProject.id + "/measures", { method: "POST", body: { measure_ids: ids } }).then(function (p) {
            ...
            var descEl = document.getElementById("pj-desc");
    ...
    // AI ASSISTANT
    // ═══════════════════════════════════════════════════════════════
    function _aiCall(systemPrompt, userPrompt) {
    ...
    var _aiSuggestedMeasures = [];
    var _aiSuggestedProjects = [];
    var _aiSuggestProjectId = null;
    window._aiSuggestMeasures = function (projectId) {
        if (!_editingProject)
            return;
    ...
    function _aiRenderSuggestedMeasures(projectName) {
        var p = _aiEnsurePanel();
        ...
        _aiSuggestedMeasures.forEach(function (r, i) {
            ...
            if (m.entity_name && m.entity_name !== m.title)
                h += ' &middot; ' + esc(m.entity_name);
            h += '</div>';
            if (r.reason)
                h += '<div class="ai-card-reason">' + esc(r.reason) + '</div>';
            h += '</div></div></div>';
        });
        p.body.innerHTML = h;
        p.footer.innerHTML =
    ...
    window._aiToggleAllMeasures = function () {
        ...
        cbs.forEach(function (cb) { if (!cb.checked)
            anyUnchecked = true; });
        cbs.forEach(function (cb) { cb.checked = anyUnchecked; });
    };
    window._aiAddSuggestedMeasures = function () {
        if (!_aiSuggestProjectId)
            return;
        var cbs = document.querySelectorAll(".ai-meas-cb:checked");
        if (!cbs.length) {
            showStatus(t("pilot.measures.none_selected"));
            return;
        }
    ...
    function _aiRenderSuggestedPlan() {
        ...
        if (validAdditions.length) {
            ...
            validAdditions.forEach(function (a, i) {
                ...
                h += '</div>';
                if (a.reason)
                    h += '<div class="ai-card-reason ct-mt-1">' + esc(a.reason) + '</div>';
                h += '</div></div></div>';
            });
        }
        if (validNewProjects.length) {
            ...
            validNewProjects.forEach(function (np, i) {
                ...
                if (np.priority)
                    h += ' <span class="ct-badge ct-ml-1" data-tone="' + _pilotTone(np.priority) + '">' + tEsc("pilot.project.priority." + np.priority) + '</span>';
                h += '</div>';
                if (np.description)
                    h += '<div class="ai-card-reason" style="font-style:normal;margin-bottom:var(--ct-s1)">' + esc(np.description) + '</div>';
    ...
    window._aiApplySuggestedPlan = function () {
        var addCbs = document.querySelectorAll(".ai-add-cb:checked");
        var newCbs = document.querySelectorAll(".ai-newproj-cb:checked");
        if (!addCbs.length && !newCbs.length) {
            showStatus(t("pilot.ai.no_action_selected"));
            return;
        }
        var pickedAdditions = [];
        addCbs.forEach(function (cb) {
            var idx = parseInt(cb.value);
            if (_aiSuggestedAdditions[idx])
                pickedAdditions.push(_aiSuggestedAdditions[idx]);
        });
        var pickedNew = [];
        newCbs.forEach(function (cb) {
            var idx = parseInt(cb.value);
            if (_aiSuggestedProjects[idx])
                pickedNew.push(_aiSuggestedProjects[idx]);
        });
        ...
        chain.then(function () {
            ...
            var msg = [];
            if (addedCount)
                msg.push(t("pilot.measures.added_n", { n: addedCount }));
            if (createdCount)
                msg.push(t("pilot.projects.created_n", { n: createdCount }));
    ...
    function _doRenderDirectory(c) {
        ...
        if (!_directory.length) {
            h += '<div class="ct-muted ct-p-5 ct-ta-c">' + t("pilot.directory.empty") + '</div>';
            c.innerHTML = h;
            return;
        }
    ...
    function _dirOpenPersonModal(idx) {
        ...
        buttons.push({
            ...
            result: function () {
                ...
                ["nom", "prenom", "email", "fonction", "departement", "statut", "telephone", "site"].forEach(function (k) {
                    var el = document.getElementById("pdm-" + k);
                    data[k] = el ? (el.value || "") : "";
                });
                ...
                var mgrPerson = _directory.find(function (x) {
        ...
        ct_modal.open({
            ...
            buttons: buttons,
            onOpen: function () {
                if (locked || typeof ct_userpicker === "undefined" || !ct_userpicker.mount)
                    return;
                var mgr = _directory.find(function (x) { return !!x.email && !!p.manager_email && String(x.email).toLowerCase() === String(p.manager_email).toLowerCase(); });
    ...
    function _dirModalUserSelect(name, label, current, excludeEmail, disabled) {
        ...
        var found = false;
        _directory.forEach(function (person) {
            if (excludeEmail && person.email === excludeEmail)
    ...
    window._dirEditPerson = function (idx) { _dirOpenPersonModal(parseInt(idx)); };
    ...
    window._dirImportCsv = function () {
        var el = document.getElementById("csv-import-input");
        if (!el)
            return;
        el.value = "";
    ...
    var _MODULE_LABELS = { risk: "Risk", vendor: "Vendor", compliance: "Compl.", audit: "Audit", asset: "Asset", access: "Access", surface: "Surface", appsec: "AppSec", watch: "Watch" };
    function _doRenderUsers(c) {
    ...
    window._changeRole = function (uid, role) {
    ...
    window._deleteUser = function (uid, email) {
        if (!confirm(t("pilot.users.delete_confirm", { email: email })))
            return;
    ...
    window._changeModPerm = function (uid, mod, role) {
    ...
    var _bkSelected = {};
    function _bkSelectedKeys() {
        return Object.keys(_bkSelected).filter(function (k) { return _bkSelected[k]; });
    }
    function _bkFilterChanged(val) {
        _bkFilter = val;
        _renderPanel();
    }
    function _bkSelectTab(tab) {
        _bkTab = tab;
        _renderPanel();
    }
    function _renderBackups(c) {
        if (!window._currentUser || window._currentUser.role !== "admin") {
        ...
        if (_bkTab === "restore")
            ...
            _renderBackupsInner(inner);
    }
    function _renderBackupsInner(c) {
        ...
        if (_backupList && _backupList.length) {
            var mods = [];
            _backupList.forEach(function (b) { if (mods.indexOf(b.module) < 0)
                mods.push(b.module); });
            mods.sort();
        ...
        if (!_backupList || !_backupList.length) {
            ...
            shown.forEach(function (b) {
                ...
                h += '<td class="ct-text-meta">' + esc(date)
                    + (b.manual ? ' <span class="ct-badge" data-tone="info" title="' + esc(b.created_by || "") + '">' + t("pilot.backups.badge_manual") + '</span>' : '')
                    + '</td>';
    ...
    window._saveBackupConfig = function () {
        ...
        ["pilot", "risk", "vendor", "compliance", "audit", "asset", "access", "surface", "appsec", "watch"].forEach(function (mod) {
            var enabledEl = document.getElementById("bk-" + mod + "-enabled");
            var freqEl = document.getElementById("bk-" + mod + "-freq");
            var rdEl = document.getElementById("bk-" + mod + "-rd");
            var rwEl = document.getElementById("bk-" + mod + "-rw");
            var rmEl = document.getElementById("bk-" + mod + "-rm");
            if (!enabledEl || !freqEl || !rdEl || !rwEl || !rmEl)
                return;
            configs.push({
                module: mod,
    ...
    window._runBackup = function (mod) {
    ...
    window._downloadBackup = function (key) {
        var a = document.createElement("a");
        a.href = "/api/backups/download/" + encodeURIComponent(key);
        a.download = key + ".json";
        a.click();
    };
    window._restoreBackup = function (key, mod) {
        if (!confirm(t("pilot.backups.restore_confirm", { mod: mod })))
            return;
    ...
    window._deleteBackup = function (key) {
        if (!confirm(t("pilot.backups.delete_confirm")))
            return;
    ...
    window._bkToggleOne = function (key, el) {
        if (el.checked)
            _bkSelected[key] = true;
        else
            delete _bkSelected[key];
        _renderPanel();
    };
    window._bkToggleAll = function (el) {
        var list = _backupList || [];
        list.filter(function (b) { return _bkFilter === "all" || b.module === _bkFilter; })
            .forEach(function (b) {
            if (el.checked)
                _bkSelected[b.key] = true;
            else
                delete _bkSelected[b.key];
        });
        _renderPanel();
    };
    ...
    window._deleteSelectedBackups = function () {
        ...
        if (!keys.length)
            return;
        if (!confirm(t("pilot.backups.delete_selected_confirm", { n: keys.length })))
            return;
    ...
    function _renderSettings(c) {
        ...
        function wireLang() {
            ["fr", "en"].forEach(function (lang) {
                ...
                if (b)
                    b.onclick = function () { switchLang(lang, _renderPanel); };
            });
        }
        if (!window._currentUser || window._currentUser.role !== "admin") {
        ...
        var providerLabels = { anthropic: "Anthropic (Claude)", openai: "OpenAI (GPT)", gemini: "Google (Gemini)", custom: s.ai_custom_label || "Custom LLM" };
        ...
        var sel = document.getElementById("set-ai-provider");
        if (sel)
            sel.onchange = function () {
                var box = document.getElementById("ai-provider-fields");
                if (box) {
                    box.innerHTML = _renderAiProviderFields(this.value, _settings || {});
    ...
    window._saveSettings = function () {
        ...
        if (demoEl)
            data.demo_mode = demoEl.checked ? "true" : "false";
        ...
        if (customLabel !== null)
            data.ai_custom_label = customLabel.trim();
        ...
        if (smtpHost !== null)
            data.smtp_host = smtpHost.trim();
        if (smtpPort !== null)
            data.smtp_port = smtpPort.trim();
        if (smtpUser !== null)
            data.smtp_user = smtpUser.trim();
        if (smtpPassword && smtpPassword.indexOf("\u2022") < 0)
            data.smtp_password = smtpPassword;
        if (smtpFrom !== null)
            data.smtp_from = smtpFrom.trim();
        if (smtpTls !== null)
            data.smtp_tls = smtpTls;
    ...
    window._resyncModules = function () {
    ...
    function _syncEvidencesBackground() {
        ...
        return _fetch("/evidences/sync", { method: "POST" }).then(function (res) {
            ...
            for (var mod in report) {
                ...
                if (r.error)
                    ...
                    parts.push(mod + ": " + ((r.added || 0) + (r.updated || 0)));
            }
    ...
    function _renderEvidences(c) {
        ...
        if (_evidenceSyncStatus)
            h += '<div style="font-size:var(--ct-text-label);color:var(--ct-ink-2);margin:var(--ct-s2) 0 var(--ct-s3)">' + esc(_evidenceSyncStatus) + '</div>';
        // Transverse expiration summary — the cross-module alert.
        var counts = { expiree: 0, bientot: 0, valide: 0, na: 0 };
        _evidences.forEach(function (e) { var s = e.status || "na"; counts[s] = (counts[s] || 0) + 1; });
        // Shared ct-kpi tiles (dashboard / watch consistency).
        var evTones = { expiree: "critical", bientot: "medium", valide: "low", na: "" };
        ...
        var evMods = {};
        _evidences.forEach(function (e) { if (e.module)
            evMods[e.module] = true; });
        ...
        var evShown = _evidences.filter(function (e) {
            if (_evidenceFilter.module && e.module !== _evidenceFilter.module)
                return false;
            var q = (_evidenceFilter.search || "").toLowerCase();
            if (q) {
                var hay = ((e.label || "") + " " + (e.entity_name || "") + " " + (e.owner || "") + " "
                    + (e.tags || []).join(" ") + " "
                    + (e.linked || []).map(function (l) { return l.object_id + " " + (l.label || ""); }).join(" ")).toLowerCase();
                if (hay.indexOf(q) < 0)
                    return false;
            }
            return true;
        ...
        evShown.forEach(function (e) {
            ...
            var linked = (e.linked || []).map(function (l) { return l.object_id; }).join(", ");
    ...
    window._filterEvidences = function () {
        var s0 = document.getElementById("ev-filter-search");
        var m0 = document.getElementById("ev-filter-module");
        ...
        if (focus === "ev-filter-search") {
            var s1 = document.getElementById("ev-filter-search");
            if (s1) {
                s1.focus();
                s1.setSelectionRange(s1.value.length, s1.value.length);
            }
        }
    };
    window._editEvidence = function (module, sourceId) {
        ...
        fields.forEach(function (f) {
            ...
            if (f.type === "picker")
                body += '<div id="ev-owner-slot"></div>';
            else if (f.type === "textarea")
                body += '<textarea id="ev-' + f.key + '" rows="2" class="ct-w-full">' + esc(v) + '</textarea>';
            else
                body += '<input type="' + f.type + '" id="ev-' + f.key + '" value="' + esc(v) + '" class="ct-w-full">';
            body += '</label>';
        });
        body += '</div>';
        ct_modal.open({
            ...
            buttons: [
                ...
                { id: "save", primary: true, label: t("pilot.action.save"), result: function () {
                        ...
                        fields.forEach(function (f) {
                            ...
                            var el = document.getElementById("ev-" + f.key);
                            if (el)
                                data[f.key] = el.value;
                        });
                        return data;
            ...
            _fetch("/evidences/" + encodeURIComponent(module) + "/" + encodeURIComponent(sourceId), {
                ...
                _loadEvidences().then(function () { if (_panel === "evidences")
                    _renderPanel(); });
            }).catch(function (err) { showStatus((err && err.message) || t("pilot.common.error"), true); });
        });
    };
        ...
        _loadModules().then(function () {
            ...
            setInterval(function () {
                _syncMeasuresBackground().then(function () {
                    ...
                    if (_panel === "dashboard" || _panel === "measures")
                        _renderPanel();
                });
            }, 300000);
        }).catch(function (e) {
    ...
    var _rstWindow = null;
    var _rstModule = "";
    var _rstSession = null;
    var _rstPollTimer = null;
    var _rstDiffs = {};
    var _rstJournal = null;
    function _rstRender(c) {
        if (!_rstWindow) {
            c.innerHTML = '<div class="ct-ta-c ct-p-8 ct-muted">' + t("pilot.common.loading") + '</div>';
            _fetch("/restore/window").then(function (d) { _rstWindow = d; _rstRender(c); })
                .catch(function (e) { c.innerHTML = '<div class="ct-p-5 ct-muted">' + esc(e.message || String(e)) + '</div>'; });
            return;
        }
        ...
        if (winSel.from) {
            ...
            var pl = function (n) { return (n < 10 ? "0" : "") + n; };
        ...
        if (_rstJournal === null) {
            _fetch("/restore/journal/" + encodeURIComponent(mod)).then(function (d) {
                _rstJournal = d || [];
                if (_panel === "backups" && _bkTab === "restore")
                    _renderPanel();
            ...
            _rstJournal.slice(0, 15).forEach(function (e) {
        ...
        c.innerHTML = h;
        if (_rstSession && _rstSession.status === "preparing")
            _rstSchedulePoll();
    }
    function _rstFmtDT(iso) {
        if (!iso)
            return "—";
        try {
    ...
    function _rstObjectsTable(s) {
        ...
        (s.objects || []).forEach(function (o) {
            var badge = "";
            if (o.missing_live)
                badge = '<span class="ct-badge" data-tone="critical">' + t("pilot.restore.deleted_since") + '</span>';
            else if (o.created_after_t)
                badge = '<span class="ct-badge" data-tone="info">' + t("pilot.restore.created_after") + '</span>';
            else if (o.suspect)
                badge = '<span class="ct-badge" data-tone="high">' + t("pilot.restore.modified_since") + '</span>';
            else
                badge = '<span class="ct-badge" data-tone="low">' + t("pilot.restore.identical") + '</span>';
            ...
            if (d && !d.diff.identical) {
                ...
                (d.diff.collections || []).forEach(function (r) {
                    ...
                    if ((ex.removed || []).length)
                        parts.push(t("pilot.restore.col_removed") + " : " + ex.removed.map(esc).join(", "));
                    if ((ex.changed || []).length)
                        parts.push(t("pilot.restore.col_changed") + " : " + ex.changed.map(esc).join(", "));
                    if ((ex.added || []).length)
                        parts.push(t("pilot.restore.col_added") + " : " + ex.added.map(esc).join(", "));
                    if (parts.length) {
                ...
                if ((d.journal || []).length) {
                    ...
                    h += d.journal.slice(0, 5).map(function (e) {
    ...
    function _rstSumDiff(d, key) {
        var n = 0;
        (d.diff.collections || []).forEach(function (r) { n += r[key] || 0; });
        return n;
    }
    ...
    function _rstModuleChanged(val) {
        _rstModule = val;
        _rstSession = null;
        _rstDiffs = {};
        _rstJournal = null;
    ...
    function _rstExploreBefore(loggedAt) {
        ...
        var mod = document.getElementById("restore-module").value;
        _rstModule = mod;
        _rstDiffs = {};
    ...
    function _rstExplore() {
        var mod = document.getElementById("restore-module").value;
        var timeRaw = document.getElementById("restore-time").value;
        var time = null;
        if (timeRaw) {
    ...
    function _rstSchedulePoll() {
        if (_rstPollTimer)
            window.clearTimeout(_rstPollTimer);
        _rstPollTimer = window.setTimeout(function () {
            if (!_rstSession || _panel !== "backups" || _bkTab !== "restore")
                return;
            _fetch("/restore/sessions/" + encodeURIComponent(_rstModule)).then(function (d) {
                _rstSession = d;
                _renderPanel();
            }).catch(function () { _rstSchedulePoll(); });
        }, 3000);
    }
    function _rstObjDiff(id) {
        _fetch("/restore/sessions/" + encodeURIComponent(_rstModule) + "/objects/" + encodeURIComponent(id) + "/diff")
            .then(function (d) { _rstDiffs[id] = d; _renderPanel(); })
            .catch(function (e) { showStatus(e.message || String(e), true); });
    }
    function _rstObjApply(id, name) {
        _confirmDialog(t("pilot.restore.confirm_title", { name: name }), t("pilot.restore.confirm_body"), { yes: t("pilot.restore.confirm_yes"), no: t("btn_cancel") }).then(function (ok) {
            if (!ok)
                return;
            ...
            _fetch("/restore/sessions/" + encodeURIComponent(_rstModule) + "/objects/" + encodeURIComponent(id) + "/restore", { method: "POST", body: {} })
                ...
                delete _rstDiffs[id];
                _fetch("/restore/sessions/" + encodeURIComponent(_rstModule)).then(function (d) { _rstSession = d; _renderPanel(); });
            })
                .catch(function (e) { showStatus(e.message || String(e), true); });
        });
    }
    function _rstPromote() {
        ...
        if (typed === null)
            return;
        if (typed !== mod) {
            showStatus(t("pilot.restore.n2_mismatch"), true);
            return;
        }
        showStatus(t("pilot.restore.restoring"));
    ...
    window._rstModuleChanged = _rstModuleChanged;
    window._rstExplore = _rstExplore;
    window._rstExploreBefore = _rstExploreBefore;
    window._rstRefreshJournal = _rstRefreshJournal;
    window._bkSelectTab = _bkSelectTab;
    window._bkFilterChanged = _bkFilterChanged;
    window._rstObjDiff = _rstObjDiff;
    window._rstObjApply = _rstObjApply;
    window._rstPromote = _rstPromote;
    window._rstCloseSession = _rstCloseSession;
    _initAuth();
})();
