...
function _renderMeasures(c: HTMLElement) {
    ...
    var groupsShown = _groups.filter(function(g) {
        if (_measureFilter.module) return (g.members || []).some(function(mm) { return mm.module === _measureFilter.module; });
        return true;
    });
...
window._searchMeasuresToAdd = function(val: string) {
    ...
    if (!results.length) { el.innerHTML = '<div class="ct-muted ct-text-meta">' + t("pilot.common.no_results") + '</div>'; return; }
...
function _doRenderUsers(c: HTMLElement) {
    ...
    _users.forEach(function(u) {
        ...
        if (u.picture) h += '<img src="' + esc(u.picture) + '" style="width:20px;height:20px;border-radius:50%">';
