...
(function() {
    ...
    var _kpiData: PilotKpi[] | null = null;       // last GET /api/kpis response
    var _filterCat = "all";
    var _filterFw = "all";
    var _filterSrc = "all";
    var _filterStatus = "active"; // 'all' | 'active' | 'inactive'

    function _catLabel(c: string): string { return t("pilot.kpi.cat." + c); }
    ...
    function _kpiName(k: PilotKpi): string { return (_locale === "en" && k.name_en) ? k.name_en : k.name_fr; }
    function _kpiDesc(k: PilotKpi): string { return (_locale === "en" && k.description_en) ? k.description_en! : (k.description_fr || ""); }
    var CAT_ORDER = ["govern", "identify", "protect", "detect", "respond", "recover"];
    var FW_LABELS: Record<string, string> = {
    ...
    function _kpiHealth(k: PilotKpi): PilotKpiHealth {
        if (k.latest == null) return { color: "grey", label: t("pilot.kpi.status.no_data") };
        ...
        var cc: any = k.connector_config;
        if (cc && cc.severity) {
            if (cc.severity === "red") return { color: "red", label: t("pilot.kpi.status.critical") };
            if (cc.severity === "amber") return { color: "amber", label: t("pilot.kpi.status.overdue") };
        ...
        if (k.direction === "higher_better") {
            if (r != null && v < r) return { color: "red", label: t("pilot.kpi.status.critical") };
            if (a != null && v < a) return { color: "amber", label: t("pilot.kpi.status.watch") };
            if (tgt != null && v < tgt) return { color: "amber", label: t("pilot.kpi.status.below_target") };
            return { color: "green", label: t("pilot.kpi.status.compliant") };
        } else {
            if (r != null && v > r) return { color: "red", label: t("pilot.kpi.status.critical") };
            if (a != null && v > a) return { color: "amber", label: t("pilot.kpi.status.watch") };
            if (tgt != null && v > tgt) return { color: "amber", label: t("pilot.kpi.status.above_target") };
            return { color: "green", label: t("pilot.kpi.status.compliant") };
        }
    }

    function _fmtValue(k: PilotKpi): string {
        if (k.latest == null) return "—";
        var v = k.latest.value;
        if (k.unit === "%") return v.toFixed(1) + " %";
        if (k.unit === "days") return v.toFixed(1) + " j";
        if (k.unit === "count") return Math.round(v).toString();
        if (k.unit === "currency") return v.toLocaleString() + " €";
        return v.toString();
    }
    function _fmtThreshold(v: number | null | undefined, unit: string): string {
        if (v == null) return "—";
        if (unit === "%") return v + " %";
    ...
    function _fmtSource(src: string | undefined): string {
        if (!src) return "";
        if (src === "auto") return t("pilot.kpi.src_auto_short");
        if (src.indexOf("manual:") === 0) return t("pilot.kpi.src_manual") + " · " + src.substring(7);
        if (src.indexOf("plugin:") === 0) return t("pilot.kpi.src_plugin") + " · " + src.substring(7);
        return src;
    }
    function _isAdmin() {
    ...
    var _aiProviderCache: string | null | undefined;
    function _aiProviderP(): Promise<string | null> {
        if (_aiProviderCache !== undefined) return Promise.resolve(_aiProviderCache);
        return _fetch("/ai/config").then(function(cfg: any) {
    ...
    function _parseJsonArray(txt: string): any[] {
        if (!txt) return [];
    ...
    function _kpiByCode(code: string): PilotKpi | undefined {
        return (_kpiData || []).filter(function(k) { return k.code === code; })[0];
    }
    function _numOrNull(id: string): number | null {
        var v = (document.getElementById(id) as HTMLInputElement).value;
        if (v === "") return null;
        var n = parseFloat(v);
    ...
    function _renderOverview() {
        ...
        var counts: Record<PilotKpiColor, number> = { green: 0, amber: 0, red: 0, grey: 0 };
        var byCat: Record<string, { code: string; color: PilotKpiColor; name: string }[]> = {};
    ...
    // MAIN RENDER
    // ─────────────────────────────────────────────────────────────────
    window._renderKpis = function _renderKpis(c: HTMLElement) {
        if (_kpiData == null) {
        ...
        if (_isAdmin()) {
            ...
            if (inactAuto > 0) h += ' <span class="ct-badge kpi-btn-badge" data-fill data-tone="info">' + inactAuto + '</span>';
            h += '</button>';
        ...
        var visible = _kpiData.filter(function(k) {
            if (_filterStatus === "active" && !k.active) return false;
            if (_filterStatus === "inactive" && k.active) return false;
            if (_filterCat !== "all" && k.category_primary !== _filterCat) return false;
            if (_filterSrc !== "all" && k.source_type !== _filterSrc) return false;
            if (_filterFw !== "all" && !(k.mappings || []).some(function(m) { return m.framework === _filterFw; })) return false;
            return true;
        });
        ...
        var byCat: Record<string, PilotKpi[]> = {};
    ...
    function _kpiCard(k: PilotKpi): string {
        ...
        if (!k.active) h += '<span class="ct-kpi-adv-pill">' + t("pilot.kpi.inactive_badge") + '</span>';
        h += '</div>';
    ...
    // FILTERS
    // ─────────────────────────────────────────────────────────────────
    window._kpiSetFilter = function(field: string, value: string) {
        ...
        if (field === "cat")    _filterCat    = value;
        else if (field === "fw")     _filterFw     = value;
        else if (field === "src")    _filterSrc    = value;
        else if (field === "status") _filterStatus = value;
    ...
    // ─────────────────────────────────────────────────────────────────
    window._kpiOpenDetail = function(code: string) {
        ...
        Promise.all([snapsP, awP, aiP]).then(function(res: any[]) {
            var snaps: PilotKpiSnapshot[] = res[0] || [];
            var aw = res[1];
            var aiProvider: string | null = res[2] || null;
            var health = _kpiHealth(k!);
    ...
    function _kpiAwarenessSection(aw: any, campaignName: string): string {
        var camps = (aw && aw.campaigns) || [];
        var c: any = null;
        for (var i = 0; i < camps.length; i++) { if (camps[i].name === campaignName) { c = camps[i]; break; } }
        if (!c) return '';
        ...
        if (!late.length && !ov.length) {
            ...
            late.forEach(function(em: string) {
    ...
    function _renderDetailBody(k: PilotKpi, health: PilotKpiHealth, snaps: PilotKpiSnapshot[], awarenessHtml: string, aiProvider: string | null) {
        var h = '';
        ...
        if (awarenessHtml) h += awarenessHtml;
    ...
    // ─────────────────────────────────────────────────────────────────
    window._kpiToggleActive = function(code: string, newValue: boolean) {
        if (!_isAdmin()) return;
    ...
    window._kpiDetailSubmitManual = function(code: string) {
        var v = (document.getElementById("kpi-detail-value") as HTMLInputElement).value;
        if (v === "" || isNaN(parseFloat(v))) {
            ct_modal.alert({ title: t("pilot.kpi.invalid_value"), message: t("pilot.kpi.enter_number") });
            return;
        }
        var ts = (document.getElementById("kpi-detail-ts") as HTMLInputElement).value;
        var note = (document.getElementById("kpi-detail-note") as HTMLInputElement).value;
        var payload: Record<string, any> = { value: parseFloat(v) };
        if (ts) payload.captured_at = new Date(ts).toISOString();
        if (note) payload.note = note;
        _fetch("/kpis/" + encodeURIComponent(code) + "/manual", {
            method: "POST",
            body: payload
        }).then(function(r) {
            if (r && r.idempotent) showStatus(t("pilot.kpi.value_already_saved"));
            else showStatus(t("pilot.kpi.value_saved"));
            return _loadKpis().then(function() {
                if (window.ct_modal && ct_modal.close) ct_modal.close();
                window._renderKpis!(document.getElementById("content")!);
    ...
    window._kpiDetailSaveTune = function(code: string) {
        var payload = {
    ...
    window._kpiDetailDelete = function(code: string) {
        ct_modal.confirm({
    ...
    window._kpiRemoveMapping = function(code: string, idx: number) {
        if (!_isAdmin()) return;
        var k = _kpiByCode(code);
        if (!k) return;
    ...
    window._kpiAddMapping = function(code: string) {
        if (!_isAdmin()) return;
        var k = _kpiByCode(code);
        if (!k) return;
        var fw = (document.getElementById("kpi-detail-add-fw") as HTMLSelectElement).value;
        var ref = ((document.getElementById("kpi-detail-add-ref") as HTMLInputElement).value || "").trim();
        var label = ((document.getElementById("kpi-detail-add-label") as HTMLInputElement).value || "").trim();
        if (!ref) {
            ct_modal.alert({ title: t("pilot.kpi.ref_missing_title"), message: t("pilot.kpi.ref_missing_msg") });
            return;
        }
        var newMappings: PilotKpiMapping[] = (k.mappings || []).map(_mappingToPayload);
        newMappings.push({
            framework: fw,
            ref: ref,
            label_fr: label || null,
    ...
    function _mappingToPayload(m: PilotKpiMapping): PilotKpiMapping {
        return {
    ...
    window._kpiSuggestFrameworks = function(code: string, provider: string) {
        if (!_isAdmin()) return;
        var k = _kpiByCode(code);
        if (!k) return;
        var out = document.getElementById("kpi-detail-ai-suggestions");
        if (out) out.innerHTML = '<span class="ct-muted ct-text-data">' + esc(t("pilot.kpi.ai_analyzing")) + '</span>';
        ...
        .then(function(res: any) {
            ...
            var h = '';
            var n = 0;
            arr.forEach(function(s: any) {
                if (!s || !s.framework || !s.ref) return;
                if (FW_ORDER.indexOf(s.framework) < 0) return;
                if (seen[s.framework + "|" + s.ref]) return;
                n++;
                ...
                if (s.label) h += '<span class="ct-muted">' + esc(String(s.label)) + '</span>';
                h += '</label>';
            });
            if (n) {
            ...
            if (out) out.innerHTML = h || '<span class="ct-muted ct-text-data">' + esc(t("pilot.kpi.ai_no_new_suggestions")) + '</span>';
        })
        .catch(function(e: any) {
            if (out) out.innerHTML = '<span style="color:var(--danger,var(--ct-critical));font-size:var(--ct-text-data)">' + esc(t("pilot.kpi.ai_error")) + ' : ' + esc(String(e && e.message || e)) + '</span>';
        });
    };

    // Add ALL checked AI-suggested mappings at once (single KPI PATCH).
    window._kpiAddSelectedMappings = function(code: string) {
        if (!_isAdmin()) return;
        var k = _kpiByCode(code);
        if (!k) return;
        ...
        }).catch(function(e: any) { ct_modal.alert({ title: t("pilot.kpi.error"), message: String(e) }); });
    };
    ...
    // ─────────────────────────────────────────────────────────────────
    window._kpiOpenHistory = function(code: string) {
        var k = _kpiByCode(code);
        if (!k) return;
        _fetch("/kpis/" + encodeURIComponent(code) + "/snapshots?limit=500").then(function(snaps: PilotKpiSnapshot[] | null) {
            snaps = snaps || [];
    ...
    window._kpiPickerActivate = function(code: string) {
        if (!_isAdmin()) return;
        _fetch("/kpis/" + encodeURIComponent(code), {
            method: "PATCH",
            body: { active: true }
        ...
        ct_modal.open({
            ...
            buttons: [
                { id: "cancel", label: t("pilot.action.cancel") },
                {
                    ...
                    result: function() {
                        var code = ((document.getElementById("kpi-new-code") as HTMLInputElement).value || "").trim();
                        var nameFr = ((document.getElementById("kpi-new-name-fr") as HTMLInputElement).value || "").trim();
                        var nameEn = ((document.getElementById("kpi-new-name-en") as HTMLInputElement).value || "").trim() || nameFr;
                        if (!code || !nameFr) {
                        ...
                        var payload = {
                            ...
                            category_primary: (document.getElementById("kpi-new-cat") as HTMLSelectElement).value,
                            unit: (document.getElementById("kpi-new-unit") as HTMLSelectElement).value,
                            direction: (document.getElementById("kpi-new-dir") as HTMLSelectElement).value,
                            source_type: (document.getElementById("kpi-new-src") as HTMLSelectElement).value,
