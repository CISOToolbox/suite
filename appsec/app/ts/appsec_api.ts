var AppSecAPI: AppSecApiType = (function() {
    "use strict";
    var BASE = "api";

    async function _fetch(path: string, opts?: AppSecFetchOpts): Promise<any> {
        opts = opts || {};
    ...
    return {
        ...
        getApp: function(id: string | number | null) { return _fetch("/applications/" + id); },
        createApp: function(data: AppSecAppPayload) { return _fetch("/applications", { method: "POST", body: data }); },
        updateApp: function(id: string | number, data: AppSecAppPayload) { return _fetch("/applications/" + id, { method: "PATCH", body: data }); },
        deleteApp: function(id: string | number) { return _fetch("/applications/" + id, { method: "DELETE" }); },
        triggerScan: function(id: string | number) { return _fetch("/applications/" + id + "/scan", { method: "POST" }); },

        getFinding: function(id: string | null) { return _fetch("/findings/" + id); },
        listFindings: function(params?: AppSecQueryParams) {
            var qs = Object.entries(params || {}).filter(function(e): e is [string, string | number | boolean] {return e[1] !== null && e[1] !== undefined && e[1] !== "";}).map(function(e){return e[0]+"="+encodeURIComponent(e[1]);}).join("&");
            return _fetch("/findings" + (qs ? "?" + qs : ""));
        },
        findingsStats: function(appId?: string | number) { return _fetch("/findings/stats" + (appId ? "?app_id=" + appId : "")); },
        triageFinding: function(id: string, data: Record<string, unknown>) { return _fetch("/findings/" + id, { method: "PATCH", body: data }); },
        bulkTriageFindings: function(data: AppSecBulkTriagePayload) { return _fetch("/findings/bulk-triage", { method: "POST", body: data }); },
        analyzeFinding: function(id: string, opts?: AppSecAnalyzeOpts) {
            var body: Record<string, unknown> = { finding_id: id };
            if (opts) {
        ...
        listScans: function(appId?: string | number) { return _fetch("/scans" + (appId ? "?app_id=" + appId : "")); },
        resetStuckScans: function(appId: string | number) { return _fetch("/scans/reset/" + appId, { method: "POST" }); },
        ...
        listDerogations: function(filters) {
            var parts: string[] = [];
            Object.keys(filters || {}).forEach(function(k) { if (filters![k]) parts.push(encodeURIComponent(k) + "=" + encodeURIComponent(filters![k])); });
        ...
        updateMeasure: function(id: string, data: Record<string, unknown>) { return _fetch("/measures/" + id, { method: "PATCH", body: data }); },
        deleteMeasure: function(id: string) { return _fetch("/measures/" + id, { method: "DELETE" }); },

        listSBOM: function(params?: AppSecQueryParams) {
            var qs = Object.entries(params || {}).filter(function(e): e is [string, string | number | boolean] {return e[1] !== null && e[1] !== undefined && e[1] !== "";}).map(function(e){return e[0]+"="+encodeURIComponent(e[1]);}).join("&");
