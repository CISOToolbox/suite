...
function _icon(name, size, cls) {
    var p = _ICON_PATHS[name];
    if (!p) {
        var _sh = window.CT_ICONS;
        if (_sh)
            p = _sh[name];
    } // fall back to shared CT_ICONS (globe, moon, sun, settings…)
    if (!p)
        return "";
    var sz = size || 16;
    var c = cls ? ' class="' + cls + '"' : '';
...
var _selectedApp = null;
var _findingsFilter = { app_id: "", severity: "", scanner: "", status: "new", q: "", patch: "" };
var _sbomFilter = { app_id: "", ecosystem: "", q: "", vulnerable_only: false };
...
var _appsView = (function () {
    try {
        return localStorage.getItem("appsec.appsView") || "cards";
    }
    catch (e) {
        return "cards";
    }
...
window._appsSetView = function (view) {
    _appsView = view === "table" ? "table" : "cards";
    try {
        localStorage.setItem("appsec.appsView", _appsView);
    }
    catch (e) { }
    renderPanel();
};
window._appsSearch = function (value) {
    _appsQuery = value || "";
    ...
    if (box)
        box.innerHTML = _appsListHtml();
    var count = document.getElementById("apps-count");
    if (count)
        count.textContent = "(" + _appsMatching().length + ")";
};
// ═══════════════════════════════════════════════════════════════
// HELPERS
// ═══════════════════════════════════════════════════════════════
...
function _statusBadge(s) { return badgeTone(t("findings.status_" + s) || t("scans.status_" + s) || s, _appsecTone(s)); }
function _scannerLabel(s) { return t("scanner." + s) || s; }
function _scannerBadge(s) { return badgeTone(_scannerLabel(s), _appsecTone(s)); }
...
function _measureStatusBadge(statut) {
    ...
    return badgeTone(label, tones[statut] || "neutral");
}
...
function renderPanel() {
    ...
    switch (_panel) {
        case "dashboard":
            ...
            break;
        case "applications":
            _selectedFinding ? _renderFindingDetail(c) : (_selectedApp ? _renderAppDetail(c) : _renderApplications(c));
            break;
        case "findings":
            _selectedFinding ? _renderFindingDetail(c) : _renderFindings(c);
            break;
        case "sbom":
            _renderSBOM(c);
            break;
...
function _renderDashboard(c) {
    ...
    if (appKeys.length > 0) {
        ...
        sevs.forEach(function (sv) { if (sv !== "info")
            h += '<th class="ct-ta-c">' + _sevBadge(sv) + '</th>'; });
...
// ═══════════════════════════════════════════════════════════════
function _renderApplications(c) {
...
function _appsListHtml() {
    var apps = _appsMatching();
    if (_apps.length === 0)
        return '<p class="text-muted">' + t("apps.no_apps") + '</p>';
    if (apps.length === 0)
        return '<p class="text-muted">' + t("apps.no_match") + '</p>';
    return _appsView === "table" ? _appsTableHtml(apps) : _appsCardsHtml(apps);
}
function _appsTableHtml(apps) {
    ...
    apps.forEach(function (a) {
        ...
        if (!a.enabled)
            h += ' <span class="ct-badge" data-tone="neutral">OFF</span>';
        if (a.repo_url)
            h += '<div class="ct-text-meta ct-muted">' + esc(a.repo_url) + '</div>';
        h += '</td>';
        h += '<td><span class="ct-badge" data-tone="' + _appsecTone(a.criticality || "medium") + '">' + esc(a.criticality || "medium") + '</span></td>';
        h += '<td>';
        if (hasFindings) {
            if (a.findings_critical)
                h += badgeTone(a.findings_critical + ' C', 'critical');
            if (a.findings_high)
                h += badgeTone(a.findings_high + ' H', 'high');
            if (a.findings_medium)
                h += badgeTone(a.findings_medium + ' M', 'medium');
            if (a.findings_low)
                h += badgeTone(a.findings_low + ' L', 'low');
        }
        else {
...
function _appsCardsHtml(apps) {
    var h = '';
    {
        ...
        for (var i = 0; i < apps.length; i++) {
            ...
            h += '</div>';
            if (a.description)
                h += '<div class="app-card-label">' + esc(a.description) + '</div>';
            if (a.repo_url)
                h += '<div class="app-card-url">' + esc(a.repo_url) + '</div>';
...
window._dashNav = function (panel) { selectPanel(panel); };
window._dashNavSev = function (sev) {
...
// ═══════════════════════════════════════════════════════════════
async function _renderAppDetail(c) {
    var app;
    try {
        app = await AppSecAPI.getApp(_selectedApp);
    }
    catch (e) {
        c.innerHTML = '<p class="text-muted">Error: ' + esc(e.message) + '</p>';
        return;
    }
    var h = '<div class="ct-row ct-row-wrap ct-mb-3">';
    ...
    if (hasFinding) {
        ...
        if (app.findings_critical)
            h += badgeTone(app.findings_critical + ' Critical', 'critical');
        if (app.findings_high)
            h += badgeTone(app.findings_high + ' High', 'high');
        if (app.findings_medium)
            h += badgeTone(app.findings_medium + ' Medium', 'medium');
        if (app.findings_low)
            h += badgeTone(app.findings_low + ' Low', 'low');
        h += '</div>';
    }
    h += '</div>';
...
var _appDetailFilter = { severity: "", scanner: "", status: "", q: "" };
// Pill-based filters re-render the full panel so .active updates.
window._adSetSev = function (v) { _appDetailFilter.severity = v; renderPanel(); };
window._adSetScanner = function (v) { _appDetailFilter.scanner = v; renderPanel(); };
window._adSetStatus = function (v) { _appDetailFilter.status = v; renderPanel(); };
// Search only refreshes the table body.
window._adSetSearch = function (v) { _appDetailFilter.q = v; _refreshAppDetailFindings(); };
async function _refreshAppDetailFindings() {
    var el = document.getElementById("app-detail-findings");
    if (!el)
        return;
    var params = { app_id: _selectedApp, limit: 300 };
    var adf = _appDetailFilter;
    if (adf.severity)
        params.severity = adf.severity;
    if (adf.scanner)
        params.scanner = adf.scanner;
    if (adf.status)
        params.status = adf.status;
    if (adf.q)
        params.q = adf.q;
    try {
            ...
            fh = ct_table.render({
                ...
                columns: [
                    ...
                    { key: "patch", label: t("findings.col_patch") || "Patch", width: "132px",
                        render: function (f) { return _patchBadge(f); } },
...
window._editAppDialog = function (id) {
    var app = _apps.find(function (a) { return String(a.id) === String(id); });
    if (app)
        _showAppModal(app);
};
function _closeAppModal() { document.getElementById("app-modal-overlay").hidden = true; }
window._closeAppModal = _closeAppModal;
window._toggleImageSection = function () {
    var on = document.getElementById("app-image-enabled").checked;
    document.getElementById("app-image-fields").style.display = on ? "" : "none";
};
function _showAppModal(app) {
    var isEdit = !!app;
    ...
    h += '<div class="ct-field"><label class="appsec-field-lbl">' + t("apps.scan_paths") + '</label><textarea class="ct-input" id="app-scan-paths" rows="2" placeholder="risk&#10;shared">' + esc(isEdit && app.scan_paths ? app.scan_paths.join("\n") : "") + '</textarea><div class="appsec-field-help">' + t("apps.scan_paths_hint") + '</div></div>';
    ...
    overlay.onclick = function (e) { if (e.target === overlay && _mdTarget === overlay)
        _closeAppModal(); };
}
window._saveApp = async function (appId) {
    var scanners = [];
    document.querySelectorAll("#app-modal .scanner-check input:checked").forEach(function (el) { scanners.push(el.value); });
    var imageEnabled = document.getElementById("app-image-enabled").checked;
    if (imageEnabled)
        scanners.push("trivy_image");
    var data = {
        name: document.getElementById("app-name").value.trim(),
        description: document.getElementById("app-desc").value.trim(),
        repo_url: document.getElementById("app-repo").value.trim(),
        repo_branch: document.getElementById("app-branch").value.trim() || "main",
        scan_paths: document.getElementById("app-scan-paths").value.split("\n").map(function (s) { return s.trim().replace(/^\/+|\/+$/g, ""); }).filter(Boolean),
        docker_images: imageEnabled ? document.getElementById("app-images").value.split("\n").map(function (s) { return s.trim(); }).filter(Boolean) : [],
        scan_frequency_hours: parseInt(document.getElementById("app-freq").value) || 24,
        criticality: document.getElementById("app-crit").value,
        enabled_scanners: scanners,
        notification_emails: document.getElementById("app-notif-emails").value.split("\n").map(function (s) { return s.trim(); }).filter(Boolean),
        notification_lang: document.getElementById("app-notif-lang").value,
    };
    var token = document.getElementById("app-token").value.trim();
    if (token) {
        if (token.includes("/") || token.includes(" ") || token.includes(":")) {
            showStatus(t("apps.token_invalid"));
            return;
        }
        data.repo_token = token;
    }
    var imgToken = document.getElementById("app-image-token").value.trim();
    if (imgToken) {
    ...
    if (data.scan_paths.some(function (p) { return p.indexOf("..") !== -1; })) {
        showStatus(t("apps.scan_paths_invalid"));
        return;
    }
    try {
        if (appId)
            await AppSecAPI.updateApp(appId, data);
        else
            await AppSecAPI.createApp(data);
        _closeAppModal();
        _loadAndRender();
...
window._deleteAppFromDetail = async function (id) {
    if (!confirm(t("apps.delete_confirm")))
        return;
    try {
        await AppSecAPI.deleteApp(id);
        _selectedApp = null;
        _loadAndRender();
        showStatus(t("apps.deleted"));
    }
    catch (e) {
        showStatus("Error: " + e.message);
    }
};
window._deleteApp = async function (id) {
    if (!confirm(t("apps.delete_confirm")))
        return;
    try {
        await AppSecAPI.deleteApp(id);
...
window._triggerScan = async function (id) {
    try {
        await AppSecAPI.triggerScan(id);
        showStatus(t("apps.scan_triggered"));
    }
    catch (e) {
        showStatus("Error: " + e.message);
    }
...
window._scanAllApps = async function () {
    ...
    for (var i = 0; i < _apps.length; i++) {
        if (_apps[i].enabled) {
            try {
                await AppSecAPI.triggerScan(_apps[i].id);
                count++;
            }
            catch (e) { }
        }
...
function _renderFindings(c) {
    ...
    h += '<div class="appsec-filter ct-minw-220">'
        + (window.ctRefSelect
            ? window.ctRefSelect("f-app", f.app_id || "", _apps.map(function (a) { return { id: String(a.id), label: a.name }; }), { single: true, hideId: true,
...
async function _refreshFindingsBody() {
    var el = document.getElementById("findings-body");
    if (!el)
        return;
    try {
        var params = Object.assign({ limit: 10000 }, _findingsFilter);
        var data = await AppSecAPI.listFindings(params);
        var items = data.items || [];
        _lastFindingsItems = items;
        var h = '';
...
function _refreshCurrentFindings() {
    if (_selectedApp && document.getElementById("app-detail-findings"))
        _refreshAppDetailFindings();
    else
        _refreshFindingsBody();
}
...
function _bulkAppsecSetStatus(scope, status) {
...
window._bulkAppsecFindingsToFix = function (scope) {
    var ids = Array.from(ct_bulkbar.getSelection(scope));
    if (!ids.length)
        return;
...
window._bulkAppsecFindingsFixed = function (scope) { _bulkAppsecSetStatus(scope, "fixed"); };
window._bulkAppsecFindingsFP = function (scope) {
    var ids = Array.from(ct_bulkbar.getSelection(scope));
    if (!ids.length)
        return;
    var selected = (_lastFindingsItems || []).filter(function (f) { return ids.indexOf(f.id) >= 0; });
    ...
    ct_modal.open({
        ...
        buttons: [
            ...
            { id: "save", primary: true, label: t("btn_confirm") || t("btn_save") || "Confirmer",
                result: function () {
                    var reason = (document.getElementById("fp-bulk-reason").value || "").trim();
                    if (!reason) {
...
window._setFSev = function (v) { _findingsFilter.severity = v; renderPanel(); };
window._setFScanner = function (v) { _findingsFilter.scanner = v; renderPanel(); };
window._setFStatus = function (v) { _findingsFilter.status = v; renderPanel(); };
// Dropdown/input filters only need to refresh the table body.
window._setFApp = function (v) { _findingsFilter.app_id = v; _refreshFindingsBody(); };
...
if (window.ctRefRegister) {
    window.ctRefRegister("f-app", {
        ...
        labelFor: function (id) {
            for (var i = 0; i < _apps.length; i++) {
                if (String(_apps[i].id) === String(id))
                    return _apps[i].name;
            }
            return id;
        },
        onToggle: function (_uid, ids) {
            window._setFApp(ids && ids.length ? ids[0] : "");
        },
    });
}
window._setFSearch = function (v) { _findingsFilter.q = v; _refreshFindingsBody(); };
window._setFPatch = function (v) { _findingsFilter.patch = v; renderPanel(); };
// ═══════════════════════════════════════════════════════════════
// FINDING DETAIL (inline in content, like Surface)
// ═══════════════════════════════════════════════════════════════
window._openFinding = function (id) { _selectedFinding = id; renderPanel(); };
window._backToFindings = function () { _selectedFinding = null; renderPanel(); };
async function _renderFindingDetail(c) {
    c.innerHTML = '<p class="text-muted">' + t("ai.loading") + '</p>';
    var f;
    try {
        f = await AppSecAPI.getFinding(_selectedFinding);
    }
    catch (e) {
        c.innerHTML = '<p class="text-muted">Error: ' + esc(e.message) + '</p>';
        return;
...
var _selectedFindingObj = null;
function _runAppsecTriage(finding, status) {
    return ct_finding_view.openTriageModal(finding, status, {
        ...
        if (!payload)
            return;
        var body = { status: payload.status };
        if (payload.triage_notes != null)
            body.triage_notes = payload.triage_notes;
        if (payload.measure_title != null)
            body.measure_title = payload.measure_title;
        if (payload.measure_description != null)
            body.measure_description = payload.measure_description;
        if (payload.responsable != null)
            body.responsable = payload.responsable;
        if (payload.echeance != null)
            body.echeance = payload.echeance;
...
function _openIgnoreRuleFromFinding(finding, reason) {
    if (!window.ct_modal) {
        _backToFindings();
        return;
    }
    ...
    ct_modal.open({
        ...
        buttons: [
            ...
            { id: "save", primary: true, label: t("btn_save"),
                result: function () {
                    var reasonVal = (document.getElementById("ir-reason").value || "").trim();
                    if (!reasonVal) {
                        showStatus(t("ignore.err_required"), true);
                        return false;
                    }
                    var appIds = [];
                    var _irAll = document.getElementById("ir-all-apps");
                    if (!_irAll || !_irAll.checked) {
                        document.querySelectorAll(".ir-app-cb:checked").forEach(function (cb) { appIds.push(cb.value); });
                    }
                    var criteria = [];
                    for (var i = 0; i < _irCriteriaCount; i++) {
                        var typeEl = document.getElementById("ir-type-" + i);
                        var valEl = document.getElementById("ir-val-" + i);
                        if (!typeEl || !valEl)
                            continue;
                        var v = (valEl.value || "").trim();
                        if (v)
                            criteria.push({ type: typeEl.value, value: v });
                    }
                    if (criteria.length === 0) {
                        showStatus(t("ignore.err_no_criteria"), true);
                        return false;
                    }
...
window._appsecTriageDetail = function (status) {
    if (!_selectedFindingObj)
        return;
...
window._aiTriageFinding = function () {
    ...
    var ev = (_selectedFindingObj && _selectedFindingObj.evidence) || {};
...
window._aiTriageRun = async function () {
    ...
    if (!el)
        return;
    var ctxEl = document.getElementById("appsec-ai-context");
    var deepEl = document.getElementById("appsec-ai-deep");
    var opts = {
    ...
    try {
        var result = await AppSecAPI.analyzeFinding(_selectedFinding, opts);
        ...
        if (result.severity_recommendation)
            rh += ' ' + _sevBadge(result.severity_recommendation);
        rh += '</p>';
        ...
        if (result.summary)
            rh += '<p class="fs-sm">' + esc(result.summary) + '</p>';
        if (result.remediation)
            rh += '<div class="ct-field"><span class="appsec-field-lbl">' + esc(t("ai.remediation")) + '</span><p class="fs-sm">' + esc(result.remediation) + '</p></div>';
        if (result.references && result.references.length > 0) {
            rh += '<div class="ct-field"><span class="appsec-field-lbl">' + esc(t("ai.references")) + '</span><ul class="fs-sm">';
            result.references.forEach(function (r) { rh += '<li>' + esc(r) + '</li>'; });
            rh += '</ul></div>';
        }
        rh += '</div>';
        el.innerHTML = rh;
        ...
        el.innerHTML = '<div class="ai-error">' + esc(t("ai.error", { msg: e.message })) + '</div>';
    }
};
// ═══════════════════════════════════════════════════════════════
// SBOM
// ═══════════════════════════════════════════════════════════════
var _sbomEcosystems = []; // populated from API response
function _renderSBOM(c) {
    var f = _sbomFilter;
    var h = '<div class="ct-row ct-row-wrap ct-mb-3">';
    ...
    h += '<div class="appsec-filter ct-minw-220">'
        + (window.ctRefSelect
            ? window.ctRefSelect("s-app", f.app_id || "", _apps.map(function (a) { return { id: String(a.id), label: a.name }; }), { single: true, hideId: true,
                placeholder: t("findings.search_app"),
                emptyText: t("findings.all_apps") })
            : "")
        + '</div>';
...
async function _refreshSBOMBody() {
    var el = document.getElementById("sbom-body");
    if (!el)
        return;
    try {
        var params = Object.assign({}, _sbomFilter);
        if (params.vulnerable_only)
            params.vulnerable_only = true;
        else
            delete params.vulnerable_only;
        ...
        if (items.length === 0) {
            ...
            for (var i = 0; i < items.length; i++) {
                ...
                if (!e.direct)
                    h += ' <span class="fs-xs text-muted">(' + t("sbom.transitive") + ')</span>';
        ...
        el.innerHTML = '<p class="text-muted">Error: ' + esc(err.message) + '</p>';
    }
}
window._setSApp = function (v) { _sbomFilter.app_id = v; _refreshSBOMBody(); };
if (window.ctRefRegister) {
    window.ctRefRegister("s-app", {
        ...
        onToggle: function (_uid, ids) {
            window._setSApp(ids && ids.length ? ids[0] : "");
        },
    });
}
window._setSEco = function (v) { _sbomFilter.ecosystem = v; _refreshSBOMBody(); };
window._setSSearch = function (v) { _sbomFilter.q = v; _refreshSBOMBody(); };
window._setSVuln = function (el) { _sbomFilter.vulnerable_only = !!el.checked; _refreshSBOMBody(); };
window._sbomFilterByPkg = function (pkg) { _sbomFilter.q = pkg; _renderSBOM(document.getElementById("content")); };
// ═══════════════════════════════════════════════════════════════
// SCANS
// ═══════════════════════════════════════════════════════════════
function _renderScans(c) {
...
window._resetStuckScans = async function (appId, appName) {
    if (!confirm(t("scans.reset_confirm").replace("{name}", appName)))
        return;
    try {
...
async function _refreshIgnoreRules() {
...
function _irAppScopeHTML(selectedIds) {
...
window._irToggleAllApps = function () {
    var allChecked = document.getElementById("ir-all-apps").checked;
    var list = document.getElementById("ir-app-list");
    if (allChecked) {
        list.style.opacity = "0.4";
        list.style.pointerEvents = "none";
        document.querySelectorAll(".ir-app-cb").forEach(function (cb) { cb.checked = false; });
    }
    else {
        list.style.opacity = "";
...
window._addIgnoreRule = function () {
    ...
    ct_modal.open({
        ...
        buttons: [
            ...
            { id: "save", primary: true, label: t("btn_save"),
                result: function () {
                    var reason = (document.getElementById("ir-reason").value || "").trim();
                    if (!reason) {
                        showStatus(t("ignore.err_required"), true);
                        return false;
                    }
                    ...
                    var appIds = [];
                    var allApps = document.getElementById("ir-all-apps");
                    ...
                    if (criteria.length === 0) {
                        showStatus(t("ignore.err_no_criteria") || "Au moins un critère requis", true);
                        return false;
                    }
        ...
        AppSecAPI._fetch("/ignore-rules", { method: "POST", body: data }).then(function (resp) {
            ...
            if (resp && resp.retroactive_count)
                msg += " (" + resp.retroactive_count + " finding(s) auto-triés)";
            showStatus(msg);
            _refreshIgnoreRules();
...
window._irRemoveCrit = function (idx) {
    var row = document.getElementById("ir-crit-" + idx);
    if (row)
        row.remove();
};
window._irFilterApps = function (q) {
    q = (q || "").toLowerCase();
    document.querySelectorAll("#ir-app-list label").forEach(function (lbl) {
        lbl.style.display = !q || lbl.textContent.toLowerCase().indexOf(q) >= 0 ? "" : "none";
    });
};
window._editIgnoreRule = function (ruleId) {
    if (!window.ct_modal || !_ignoreRules)
        return;
...
window._toggleIgnoreRule = function (ruleId, enable) {
    AppSecAPI._fetch("/ignore-rules/" + ruleId, { method: "PATCH", body: { enabled: enable } }).then(function () {
        _refreshIgnoreRules();
    }).catch(function (e) { showStatus(e.message, true); });
};
window._deleteIgnoreRule = function (ruleId) {
    if (!confirm(t("ignore.confirm_delete")))
        return;
...
async function _renderAuditLog(c) {
    var retention = 365;
    try {
        var rd = await AppSecAPI._fetch("/audit-log/retention");
        retention = rd.audit_retention_days || 365;
    }
    catch (e) { }
    var h = '<div class="ct-row ct-row-wrap ct-mb-3">';
...
window._saveAuditRetention = async function () {
    var days = parseInt(document.getElementById("audit-retention").value) || 365;
    try {
...
async function _refreshAuditBody() {
    ...
    try {
        var params = {};
        if (_auditFilter.q)
            params.q = _auditFilter.q;
        var qs = Object.entries(params).filter(function (e) { return e[1]; }).map(function (e) { return e[0] + "=" + encodeURIComponent(e[1]); }).join("&");
...
// ═══════════════════════════════════════════════════════════════
var _appsecMeasures = [];
async function _renderMeasures(c) {
...
window._bulkAppsecMeasuresDone = function (scope) {
    var ids = Array.from(ct_bulkbar.getSelection(scope));
    if (!ids.length)
        return;
...
window._editAppsecMeasureRow = function (row) {
...
window._bulkAppsecMeasuresDelete = function (scope) {
    var ids = Array.from(ct_bulkbar.getSelection(scope));
    if (!ids.length)
        return;
...
if (typeof window.ct_handleMeasureDeepLink === "function") {
    window.ct_handleMeasureDeepLink({ open: function (mid) {
            if (!_appsecMeasures.some(function (m) { return m.id === mid; }))
                return false;
            if (typeof selectPanel === "function")
                selectPanel("measures");
            if (typeof window._editAppsecMeasureRow === "function")
                window._editAppsecMeasureRow({ id: mid });
            return true;
        } });
}
...
window._openNotifPrefs = function () {
    ...
    window.ct_notifprefs.open({
        ...
        savePrefs: function (prefs) { return AppSecAPI._fetch("/me/notification-prefs", { method: "PUT", body: prefs }); },
