"""Encrypt/decrypt repo & image tokens.

AES-256-GCM with a per-message random salt+nonce and PBKDF2-HMAC-SHA256
(310k iterations) key derivation — same scheme as the asset/access modules.
The key comes ONLY from ENCRYPTION_KEY (no JWT_SECRET fallback, no insecure
default): encryption fails closed if it is missing/too short.
"""
...
_MIN_KEY_LEN = 32  # 32 chars min — matches token_hex(32) = 64 chars
