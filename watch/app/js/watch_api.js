var WatchAPI = (function () {
    ...
    return {
        ...
        createScope: function (data) { return _fetch("/scopes", { method: "POST", body: data }); },
        getScope: function (id) { return _fetch("/scopes/" + encodeURIComponent(id)); },
        updateScope: function (id, data) { return _fetch("/scopes/" + encodeURIComponent(id), { method: "PATCH", body: data }); },
        deleteScope: function (id) { return _fetch("/scopes/" + encodeURIComponent(id), { method: "DELETE" }); },
        addRecipient: function (id, data) { return _fetch("/scopes/" + encodeURIComponent(id) + "/recipients", { method: "POST", body: data }); },
        removeRecipient: function (id, email) { return _fetch("/scopes/" + encodeURIComponent(id) + "/recipients/" + encodeURIComponent(email), { method: "DELETE" }); },
        // ── Phase 2: watch targets ──────────────────────────────
        listTargets: function (scopeId) { return _fetch("/scopes/" + encodeURIComponent(scopeId) + "/targets"); },
        createTarget: function (scopeId, data) { return _fetch("/scopes/" + encodeURIComponent(scopeId) + "/targets", { method: "POST", body: data }); },
        updateTarget: function (scopeId, tid, data) { return _fetch("/scopes/" + encodeURIComponent(scopeId) + "/targets/" + encodeURIComponent(tid), { method: "PATCH", body: data }); },
        deleteTarget: function (scopeId, tid) { return _fetch("/scopes/" + encodeURIComponent(scopeId) + "/targets/" + encodeURIComponent(tid), { method: "DELETE" }); },
        // ── Phase 3: alerts + feeds ─────────────────────────────
        listAlerts: function (params) {
            var qs = "";
            if (params) {
                var bits = [];
        ...
        getAlert: function (id) { return _fetch("/alerts/" + encodeURIComponent(id)); },
        setAlertStatus: function (id, data) { return _fetch("/alerts/" + encodeURIComponent(id) + "/status", { method: "PATCH", body: data }); },
        bulkSetAlertStatus: function (data) { return _fetch("/alerts/bulk-status", { method: "POST", body: data }); },
        getAlertAnalysis: function (id) {
        ...
        analyzeAlert: function (id) {
            var lang = (typeof _locale !== "undefined") ? _locale : "fr";
            return _fetch("/alerts/" + encodeURIComponent(id) + "/analyze?language=" + encodeURIComponent(lang), { method: "POST", body: {} });
        },
        getAlertSbomImpact: function (id) { return _fetch("/alerts/" + encodeURIComponent(id) + "/sbom-impact"); },
        listFeeds: function () { return _fetch("/feeds"); },
        runFeedNow: function (source) { return _fetch("/feeds/" + encodeURIComponent(source) + "/run", { method: "POST", body: {} }); },
        // ── Phase 5: digest ─────────────────────────────────────
        previewDigest: function (scopeId) {
        ...
        getDigestBody: function (runId) {
            ...
            return fetch(BASE + "/digest/runs/" + encodeURIComponent(runId) + "/body", { credentials: "same-origin" }).then(function (r) {
                ...
                if (!r.ok)
                    throw new Error("Fetch body failed");
                return r.text();
            });
        },
        sendDigestNow: function (scopeId, kind) {
