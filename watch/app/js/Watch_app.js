...
var _scopes = [];
var _directory = []; // Pilot directory cache (for recipient picker)
...
function selectPanel(name) {
    ...
    document.querySelectorAll(".ct-rail-item").forEach(function (el) {
        ...
        var match = false;
        try {
            match = !!(args && JSON.parse(args)[0] === name);
        }
        catch (e) { }
        if (match)
...
function _watchBreakdown(items, seriesFn) {
...
async function _renderDigest(c) {
    c.innerHTML = '<div class="ct-p-6 ct-muted">' + esc(t("misc.loading")) + '</div>';
    var runs = [];
    try {
        runs = await WatchAPI.listDigestRuns() || [];
    }
    catch (e) {
        runs = [];
    }
...
async function openDigestPreview() {
    var html = "";
    try {
        html = await WatchAPI.previewDigest();
    }
    catch (e) {
    ...
    ct_modal.open({
        ...
        body: iframe,
        size: "lg",
        buttons: [{ label: t("misc.close"), kind: "secondary" }]
    });
}
async function openDigestBody(runId) {
    var html = "";
    try {
        html = await WatchAPI.getDigestBody(runId);
    }
    catch (e) {
        showStatus(e.message || t("error.generic"), "error");
        return;
...
async function forceSendDigest(scopeId, kind) {
...
// ═══════════════════════════════════════════════════════════════
async function _renderScopes(c) {
    c.innerHTML = '<div class="ct-p-6 ct-muted">' + esc(t("misc.loading")) + '</div>';
    try {
    ...
    if (_scopes.length === 0) {
        ...
        if (shared.length)
            h += _scopesGroup(t("scopes.section_shared"), shared, false);
    }
    c.innerHTML = h;
}
function _scopesGroup(title, list, editable) {
...
function _scopeCard(s, editable) {
...
async function openScopeDetails(id) {
    var s;
    try {
        s = await WatchAPI.getScope(id);
    }
    catch (e) {
...
function _openScopeEditor(scope) {
    var isNew = !scope;
    ...
    var freqSel = document.getElementById("scope-threat-digest-frequency");
    if (freqSel) {
        freqSel.addEventListener("change", function () {
            ...
            if (wlbl)
                wlbl.style.display = (f === "weekly") ? "" : "none";
            if (wsel)
                wsel.style.display = (f === "weekly") ? "" : "none";
            if (trow)
                trow.style.display = (f === "off") ? "none" : "flex";
        });
    }
    // Section toggles → show/hide the associated config body.
    var vulnToggle = document.getElementById("scope-digest-enabled");
    ...
    var threatToggle = document.getElementById("scope-threat-digest-enabled");
    ...
    if (!isNew) {
        ...
        if (canEdit)
            _loadDirectoryIntoDatalist();
        _loadAndRenderTargets(scope);
    }
}
async function _loadAndRenderTargets(scope) {
...
function _renderTargetsList(box, scope, targets) {
...
async function addTargetToScope(scopeId) {
    var kind = document.getElementById("target-kind").value;
    var value = (document.getElementById("target-value").value || "").trim();
    var label = (document.getElementById("target-label").value || "").trim();
    var version = (document.getElementById("target-version").value || "").trim();
    if (!value) {
        showStatus(t("targets.error.value_required"), "err");
        return;
    }
    try {
        ...
        document.getElementById("target-version").value = "";
...
async function toggleTargetEnabled(scopeId, targetId, enabled) {
    try {
...
async function removeTargetFromScope(scopeId, targetId) {
    try {
...
function _renderRecipientsList(scope) {
...
async function _loadDirectoryIntoDatalist() {
    if (!_directoryLoaded) {
        try {
            _directory = await WatchAPI.getDirectory() || [];
        }
        catch (e) {
            _directory = [];
        }
...
async function saveScope(id) {
    ...
    var description = (document.getElementById("scope-description").value || "").trim();
    if (!name) {
        showStatus(t("scopes.error.name_required"), "err");
        return;
    }
    var enabledEl = document.getElementById("scope-digest-enabled");
    var hourEl = document.getElementById("scope-digest-hour");
    var minEl = document.getElementById("scope-digest-minute");
    var tzEl = document.getElementById("scope-digest-tz");
    var hour = parseInt((hourEl && hourEl.value), 10);
    var minute = parseInt((minEl && minEl.value), 10);
    if (isNaN(hour) || hour < 0 || hour > 23) {
        showStatus(t("scopes.error.digest_hour"), "err");
        return;
    }
    if (isNaN(minute) || minute < 0 || minute > 59) {
        showStatus(t("scopes.error.digest_minute"), "err");
        return;
    }
    // ── Thresholds (per-scope, OR semantics) ────────────────────
    var sevEl = document.getElementById("scope-severity-min");
    var kevEl = document.getElementById("scope-include-kev");
    var cvssEl = document.getElementById("scope-cvss-min");
    var epssEl = document.getElementById("scope-epss-min");
    ...
    if (cvssRaw !== "" && (isNaN(cvssVal) || cvssVal < 0 || cvssVal > 10)) {
        showStatus(t("scopes.error.cvss_min"), "err");
        return;
    }
    if (epssRaw !== "" && (isNaN(epssVal) || epssVal < 0 || epssVal > 1)) {
        showStatus(t("scopes.error.epss_min"), "err");
        return;
    }
    // ── Threat digest cadence + free-form prompt + search window ──
    var tdEnEl = document.getElementById("scope-threat-digest-enabled");
    var tdFreqEl = document.getElementById("scope-threat-digest-frequency");
    var tdWdayEl = document.getElementById("scope-threat-digest-weekday");
    var tdHourEl = document.getElementById("scope-threat-digest-hour");
    var tdMinEl = document.getElementById("scope-threat-digest-minute");
    var tdTzEl = document.getElementById("scope-threat-digest-tz");
    var tdPromptEl = document.getElementById("scope-threat-prompt");
    var tdWindowDaysEl = document.getElementById("scope-threat-window-days");
    var tdHour = parseInt((tdHourEl && tdHourEl.value), 10);
    var tdMin = parseInt((tdMinEl && tdMinEl.value), 10);
    if (isNaN(tdHour) || tdHour < 0 || tdHour > 23) {
        showStatus(t("scopes.error.digest_hour"), "err");
        return;
    }
    if (isNaN(tdMin) || tdMin < 0 || tdMin > 59) {
        showStatus(t("scopes.error.digest_minute"), "err");
        return;
    }
    var tdWindowDays = parseInt((tdWindowDaysEl && tdWindowDaysEl.value) || "7", 10);
    if (isNaN(tdWindowDays) || tdWindowDays < 1 || tdWindowDays > 30) {
        showStatus(t("scopes.error.threat_window_days"), "err");
        return;
    }
    var payload = {
        name: name,
...
async function confirmDeleteScope(id) {
    if (!confirm(t("scopes.confirm_delete")))
        return;
    try {
...
function _validEmail(v) {
    ...
    if (v.length < 3 || v.indexOf("@") < 1 || v.split("@").length !== 2)
        return false;
    var domain = v.split("@")[1];
    return !!domain && domain.indexOf(".") > 0;
}
function onRecipientInputChange(scopeId, val) {
...
async function addRecipientToScope(scopeId) {
    var inp = document.getElementById("scope-recipient-input");
    var email = (inp.value || "").trim().toLowerCase();
    if (!_validEmail(email)) {
        showStatus(t("scopes.modal.invalid_email"), "err");
        return;
    }
    ...
    var match = _directory.find(function (u) { return (u.email || "").toLowerCase() === email; });
    if (match)
        name = ((match.prenom || "") + " " + (match.nom || "")).trim() || (match.name || "");
    try {
...
async function removeRecipientFromScope(scopeId, email) {
    try {
...
function _closeModal() {
    if (typeof ct_modal !== "undefined" && ct_modal.close)
        ct_modal.close();
}
// ═══════════════════════════════════════════════════════════════
// BOOT
...
// ═══════════════════════════════════════════════════════════════
var _alertFilters = { severity: "", status: "", source: "", scope_id: "", kev_only: false, q: "" };
var _alertSearchTimer = null;
var _alertsCache = [];
...
async function _renderAlerts(c) {
...
async function _refreshAlertsRows() {
...
window._watchAlertsBulkToggle = function (alertId) {
    if (window.ct_bulkbar)
        ct_bulkbar.toggle("watch-alerts", alertId);
};
// Header checkbox: select/deselect every row currently in the DOM.
window._watchAlertsBulkToggleAll = function () {
    if (!window.ct_bulkbar)
        return;
    var boxes = document.querySelectorAll('input[type="checkbox"][data-bulk-scope="watch-alerts"][data-bulk-key]');
    var allBox = document.querySelector('input[type="checkbox"][data-bulk-scope="watch-alerts"][data-bulk-all]');
    var checked = allBox ? allBox.checked : true;
    var keys = [];
    if (checked) {
...
window._watchAlertsBulkSetStatus = function (scope, status) {
    if (!window.ct_bulkbar)
        return;
    var ids = Array.from(ct_bulkbar.getSelection(scope));
    if (!ids.length)
        return;
    WatchAPI.bulkSetAlertStatus({ ids: ids, status: status }).then(function (r) {
        var label = t("alerts.status." + status) || status;
        var msg = (r.updated || 0) + " " + (t("alerts.bulk.applied") || "alerte(s) → ") + label;
        if (r.skipped)
            msg += " (" + r.skipped + " " + (t("alerts.bulk.skipped") || "ignorée(s)") + ")";
...
function setAlertFilter(field, val) {
    _alertFilters[field] = val || "";
    _refreshAlertsRows();
}
function setAlertKev(val) {
    _alertFilters.kev_only = !!(val === true || val === "true" || val === "on");
    _refreshAlertsRows();
}
function onAlertSearch(val) {
    ...
    // user keeps typing.
    if (_alertSearchTimer)
        clearTimeout(_alertSearchTimer);
...
async function openAlertDetail(alertId) {
    ...
    var analysis = null;
    try {
        analysis = await WatchAPI.getAlertAnalysis(alertId);
    }
    catch (e) { }
    ...
    if (a.kev_listed)
        h += '<span class="ct-kpi-tone ct-text-onsolid ct-py-1 ct-px-2 ct-r-lg ct-text-label ct-strong">KEV</span>';
    if (a.cvss_score != null)
        h += '<span class="ct-text-meta ct-ink-1">CVSS ' + esc(a.cvss_score.toFixed(1)) + '</span>';
    if (a.epss_score != null)
        h += '<span class="ct-text-meta ct-ink-1">EPSS ' + esc((a.epss_score * 100).toFixed(1)) + '%</span>';
    h += '</div>';
    ...
    if (a.references_json && a.references_json.length) {
        ...
        a.references_json.forEach(function (r) {
            var url = String(r || "");
            if (!url)
                return;
    ...
    ct_modal.open({
        ...
        buttons: [
            ...
            { id: "save", label: t("misc.save"), primary: true, result: function () {
                    var status = document.getElementById("alert-status-sel").value;
                    var note = document.getElementById("alert-status-note").value;
...
async function _fillSbomImpact(alertId) {
    var body = document.getElementById("alert-sbom-body");
    if (!body)
        return;
    var data;
    try {
        data = await WatchAPI.getAlertSbomImpact(alertId);
    }
    catch (e) {
    ...
    var findings = data.matched_findings || [];
    var sbom = data.matched_sbom || [];
...
async function runAlertAnalysis() {
    ...
    if (body)
        body.innerHTML = '<div class="ct-muted">' + esc(t("alerts.detail.analyze_pending")) + '</div>';
    try {
        ...
        if (body)
            body.innerHTML = '<div class="ct-text-critical">' + esc(e.message || t("error.generic")) + '</div>';
    }
}
// ═══════════════════════════════════════════════════════════════
// SOURCES PANEL (Phase 3 — read-only for users, run-now for admin)
// ═══════════════════════════════════════════════════════════════
async function _renderSources(c) {
    c.innerHTML = '<div class="ct-p-6 ct-muted">' + esc(t("misc.loading")) + '</div>';
    var rows = [];
    try {
    ...
    rows.forEach(function (r) {
        ...
        if (isAdmin)
            h += '<td class="ct-p-2"><button class="ct-btn" data-click="runFeedNow" data-args=\'["' + esc(r.source) + '"]\'>' + esc(t("sources.run_now")) + '</button></td>';
        h += '</tr>';
    });
    h += '</tbody></table>';
    c.innerHTML = h;
}
async function runFeedNow(source) {
