...
(function () {
    ...
    function _renderSharePanel(panel, analysis) {
        ...
        ["read", "edit", "delete", "share"].forEach(function (p) {
            var labels = { read: "Lecture", edit: "Modification", "delete": "Suppression", share: "Partage" };
    ...
    function _addShare(analysisId) {
        var email = document.getElementById("share-email");
        if (!email || !email.value.trim())
            return;
        var checkboxes = document.querySelectorAll("#share-perms input[type=checkbox]");
        var perms = [];
        checkboxes.forEach(function (cb) { if (cb.checked)
            perms.push(cb.value); });
        if (perms.indexOf("read") < 0)
            perms.unshift("read");
    ...
    function _removeShare(analysisId, email) {
