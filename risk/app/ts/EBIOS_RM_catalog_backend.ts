...
var _activeId: string | null = localStorage.getItem("ebios_catalog_active") || null;
...
var _catalogFilter = "";
var _renderTimer: ReturnType<typeof setTimeout> | null = null;
...
var _sectionDirty: Record<string, boolean> = {};
var _sectionFlushTimer: ReturnType<typeof setTimeout> | null = null;

var _SECTION_MAP: Record<string, string> = {
...
function _flushSections() {
    if (_sectionFlushTimer) clearTimeout(_sectionFlushTimer);
...
function _flushNow() {
    if (!_activeId || _saveLocked) return;
    if (_sectionFlushTimer) { clearTimeout(_sectionFlushTimer); _sectionFlushTimer = null; }
    var batch = _sectionDirty; _sectionDirty = {};
    for (var section in batch) {
        var data = D[section];
        if (data === undefined) continue;
        try { RiskAPI._putSection(_activeId, _SECTION_MAP[section] || section, data, true); } catch (e) { /* best effort */ }
    }
}
...
window._persist = function(entityType: string, entityId?: unknown, fields?: unknown) {
    if (!_activeId || _saveLocked) return;
    _sectionDirty[entityType] = true;
    _flushSections();
};

window._persistCreate = function(entityType: string, data?: unknown) {
...
window._persistDelete = function(entityType: string, entityId?: unknown) {
...
window.catalogCreate = function() {
    if (!confirm(t("confirm_new", {label: t(_ct().labelKey || "analysis")}))) return;
    _lockSave();
...
window.catalogOpen = function(id: string | number) {
    if (String(id) === String(_activeId)) return;
    _lockSave();
    localStorage.setItem("ebios_catalog_active", String(id));
    window.location.reload();
};

window.catalogDelete = function(id: string | number) {
    if (!confirm(t("catalog.delete_confirm"))) return;
    _lockSave();
    RiskAPI.del(id).then(function() {
        if (String(id) === String(_activeId)) localStorage.removeItem("ebios_catalog_active");
        window.location.reload();
    }).catch(function(e) { _saveLocked = false; alert("Error: " + e.message); });
};

window.catalogDuplicate = function(id: string | number) {
...
window.catalogRename = function(id: string | number) {
...
window.catalogExport = function(id: string | number) {
    RiskAPI.get(id).then(function(a) {
        if (!a) return;
...
window.catalogSearch = function(q: string) {
    ...
    RiskAPI.list().then(function(items) {
        ...
        if (!filtered.length) {
            ...
            for (var i = 0; i < filtered.length; i++) {
                ...
                if (statsStr) h += '<span>' + statsStr + '</span>';
                h += '</div>';
                ...
                if (!isActive) h += '<button class="ct-btn ct-tip" data-size="xs" data-variant="danger" data-icon data-click="catalogDelete" data-args=\'' + _da(item.id) + '\' data-stop data-tip="' + esc(t("catalog.delete")) + '">' + _icon("trash", 14) + '</button>';
                h += '</div>';
                h += '</div>';
            }
        }
...
window._appInitCallback = function() {
    RiskAPI.list().then(function(analyses) {
        ...
        try {
            ...
            if (_dlEntity && analyses.some(function(a) { return String(a.id) === _dlEntity; })) savedId = _dlEntity;
        } catch (e) { /* ignore */ }
        ...
        if (analysis) _serverRev = (analysis as any).server_rev || 0;
...
function _handleMeasureDeepLink(): void {
    ...
    window.ct_handleMeasureDeepLink({ open: function(mid: string) {
        ...
        if (!measures.some(function(m: { id: string }) { return m.id === local; })) return false;
        if (typeof selectPanel === "function") selectPanel("measures");
        setTimeout(function() {
            if (typeof window.ct_highlightMeasureRow === "function") window.ct_highlightMeasureRow(local);
        }, 150);
        return true;
    } });
}
...
if (typeof _origLoadBuffer === "function") {
    window._loadBuffer = async function(buffer: ArrayBuffer, filename: string) {
...
function _installRenderAllHook() {
    var _ra = window.renderAll as ({ (): void; _hooked?: boolean } | undefined);
    if (_ra && !_ra._hooked) {
        var _orig = _ra;
        ...
        (window.renderAll as { _hooked?: boolean })._hooked = true;
    }
}
...
document.addEventListener("visibilitychange", function() {
    if (document.visibilityState !== "visible" || !_activeId || _saveLocked || _saveTimer) return;
    RiskAPI.get(_activeId).then(function(analysis: any) {
        if (!analysis || (analysis.server_rev || 0) === _serverRev) return;
