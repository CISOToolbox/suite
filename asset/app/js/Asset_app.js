 ...
 */
var ASSET_INIT_DATA = { assets: [], groupes: [], metadata: { organization: "", created: "" } };
window.CT_CONFIG = {
    edition: "suite",
    module: "asset",
...
window.SCHEMA_REV = 1;
var D = JSON.parse(JSON.stringify(ASSET_INIT_DATA));
var _panel = "dashboard";
var _selectedAsset = null;
var _selectedGroupe = null;
var _groupeTab = "info";
...
function selectPanel(id) {
    ...
    _groupeTab = "info";
    if (id === "measures")
        _measuresLoaded = false; // always fetch fresh on open
...
function renderPanel() {
    ...
    switch (_panel) {
        case "dashboard":
            ...
            break;
        case "assets":
            c.innerHTML = _selectedAsset !== null ? renderAssetDetail() : renderAssetList();
            break;
        case "groupes":
            c.innerHTML = _selectedGroupe !== null ? renderGroupeDetail() : renderGroupeList();
            break;
        case "dependances":
            c.innerHTML = renderDependances();
            break;
        case "echeances":
            c.innerHTML = renderEcheances();
            break;
        case "measures":
            renderMeasuresPanel(c);
            break;
        case "plugins":
            renderPluginsPanel(c);
            break;
...
function _statusLabel(s) { return t("asset.statut." + s) || s; }
...
function _echeances() {
    ...
    var out = [];
    function add(asset, kind, dateStr, lead) {
        if (!dateStr)
            return;
...
function _echeanceColor(bucket) {
    return bucket === "expired" ? "#dc2626" : bucket === "due" ? "#f97316" : "#eab308";
}
function _echeanceDaysLabel(days) {
    if (days === 0)
        return t("echeance.today");
    if (days < 0)
        return t("echeance.days_ago", { n: -days });
    return t("echeance.days_left", { n: days });
}
function _updateEcheanceBadge() {
...
window.openEcheanceAsset = function (id) {
...
function exportEcheancesIcs() {
    var items = _echeances();
    if (!items.length) {
        showStatus(t("echeance.empty"), true);
        return;
    }
    function pad(n) { return n < 10 ? "0" + n : "" + n; }
    function dval(dateStr) { return dateStr.replace(/-/g, ""); }
...
var _assetSort = null; // { key, direction: "asc"|"desc" }
var _assetHiddenCols = null; // { colKey: true } — null = not yet loaded
var _ASSET_COLS_LS_KEY = "ct_asset_hidden_cols";
function _defaultHiddenCols() {
...
function _saveHiddenCols() {
    try {
        localStorage.setItem(_ASSET_COLS_LS_KEY, JSON.stringify(_assetHiddenCols || {}));
    }
    catch (e) { /* ignore */ }
}
window._sortAssets = function (key) {
...
window._toggleAssetCol = function (key, el) {
    if (!_assetHiddenCols)
        _assetHiddenCols = {};
    if (el && typeof el.checked === "boolean") {
        if (el.checked)
            delete _assetHiddenCols[key];
        else
            _assetHiddenCols[key] = true;
    }
    _saveHiddenCols();
    renderPanel();
};
...
document.addEventListener("click", function (e) {
    ...
    if (!popup || popup.style.display === "none")
        return;
    var btn = e.target.closest('[data-click="_toggleAssetColsPopup"]');
    var inside = e.target.closest("#asset-cols-popup");
    if (!btn && !inside)
...
function filterAssetType(val) { _assetTypeFilter = val || ""; renderPanel(); }
window.filterAssetType = filterAssetType;
function filterAssetCrit(val) { _assetCritFilter = val || ""; renderPanel(); }
window.filterAssetCrit = filterAssetCrit;
function filterAssetStatut(val) { _assetStatutFilter = val || ""; renderPanel(); }
...
function renderAssetList() {
    ...
    for (var ci = 1; ci <= 5; ci++)
        h += '<option value="' + ci + '"' + (_assetCritFilter == String(ci) ? " selected" : "") + '>' + esc(_critLabel(ci)) + '</option>';
    h += '</select>';
    ...
    var filtered = D.assets.filter(function (a) {
        if (_assetTypeFilter && a.type !== _assetTypeFilter)
            return false;
        if (_assetCritFilter && String(a.criticite) != _assetCritFilter)
            return false;
        if (_assetStatutFilter && (a.statut || "actif") !== _assetStatutFilter)
            return false;
        if (_assetFilter) {
            var q = _assetFilter;
...
window._openAssetRow = function (row) {
...
window._bulkAssetsRetire = function (scope) {
...
window._bulkAssetsAddToGroup = function (scope) {
    var ids = Array.from(ct_bulkbar.getSelection(scope));
    if (!ids.length)
        return;
    if (!window.ct_modal)
    ...
    ct_modal.open({
        ...
        buttons: [
            ...
            { id: "add", primary: true, label: t("btn_add") || "Ajouter",
                result: function () {
                    var el = document.getElementById("bulk-group");
...
window._bulkAssetsEdit = function (scope) {
    var ids = Array.from(ct_bulkbar.getSelection(scope));
    if (!ids.length)
        return;
    if (!window.ct_modal)
    ...
    // OS
    h += row(t("asset.col_os") || "OS", checkbox("os")
    ...
    ct_modal.open({
            ...
            { id: "save", primary: true, label: t("btn_save") || "Enregistrer",
                result: function () {
                    ...
                    ["type", "crit", "proprio", "statut", "os"].forEach(function (k) {
                        var cb = document.getElementById("bulk-apply-" + k);
                        if (cb && cb.checked) {
                            var el = document.getElementById("bulk-" + k);
                            if (el)
                                changes[k] = el.value;
                        }
                    });
        ...
        D.assets.forEach(function (a) {
            ...
            if ("crit" in changes) {
                a.criticite = parseInt(changes.crit, 10) || 2;
                _markManual(a, "criticite");
                patch.criticite = a.criticite;
            }
            if ("proprio" in changes) {
                a.proprietaire = changes.proprio;
                _markManual(a, "proprietaire");
                patch.proprietaire = a.proprietaire;
            }
            if ("statut" in changes) {
                a.statut = changes.statut;
                _markManual(a, "statut");
                patch.statut = a.statut;
            }
            if ("os" in changes) {
                a.os = changes.os;
                _markManual(a, "os");
...
window._bulkAssetsDelete = function (scope) {
    var ids = Array.from(ct_bulkbar.getSelection(scope));
    if (!ids.length)
        return;
    var idSet = {};
...
function openAsset(idx) { _selectedAsset = parseInt(String(idx)); renderPanel(); }
...
window.openAssetTypesModal = function () {
    ...
    if (!Array.isArray(D.custom_asset_types))
        D.custom_asset_types = [];
    _renderAssetTypesModal();
};
function _renderAssetTypesModal() {
    ...
    ct_modal.open({
        ...
        buttons: [
            ...
            { id: "add", primary: true, label: (t("asset.types_add") || "Ajouter ce type"),
                result: function () {
                    var id = (document.getElementById("nt-id").value || "").trim().toLowerCase().replace(/[^a-z0-9_]/g, "_");
                    var label = (document.getElementById("nt-label").value || "").trim();
                    var labelEn = (document.getElementById("nt-label-en").value || "").trim();
                    var color = document.getElementById("nt-color").value || "var(--ct-ink-2)";
                    if (!id || !label) {
                        showStatus(t("asset.types_err_missing") || "Id et Label FR requis", true);
                        return false;
                    }
                    // Conflict with built-in?
                    if (ASSET_TYPES_BUILTIN.indexOf(id) >= 0) {
                        showStatus(t("asset.types_err_builtin") || "Cet ID est déjà pris par un type prédéfini", true);
                        return false;
                    }
...
window._deleteCustomType = function (idx) {
    var customs = D.custom_asset_types || [];
    var ct = customs[idx];
    if (!ct)
        return;
...
function renderAssetDetail() {
    ...
    var deps = (a.depends_on || []).map(function (id) {
        ...
        if (grp)
            return { id: grp.id, nom: grp.nom, badge: _badge(t("nav.groupes"), "var(--ct-ink-2)"), kind: "groupe" };
        return null;
    }).filter(Boolean);
    if (deps.length) {
    ...
    var gList = (a.groupe_ids || []).map(function (gid) { return D.groupes.find(function (x) { return x.id === gid; }); }).filter(Boolean);
    if (gList.length) {
    ...
    D.groupes.forEach(function (g) {
        if ((a.groupe_ids || []).indexOf(g.id) >= 0)
            return;
...
function _markManual(asset, field) {
    if (!asset || _MANUAL_OVERRIDE_FIELDS.indexOf(field) < 0)
        return;
    asset.sources = asset.sources || {};
    asset.sources.fields = asset.sources.fields || {};
    asset.sources.fields[field] = "manual";
}
function _getManualFields(asset) {
...
window._clearManualLocks = function () {
    if (_selectedAsset === null)
        return;
    var a = D.assets[_selectedAsset];
    if (!a || !a.sources || !a.sources.fields)
        return;
    var f = a.sources.fields;
    Object.keys(f).forEach(function (k) { if (f[k] === "manual")
        delete f[k]; });
    _save();
    renderPanel();
    showStatus(t("asset.manual_lock_cleared") || "Verrous retirés — les prochaines synchros pourront écraser ces champs");
};
function saveAssetField(field, val) {
    if (_selectedAsset === null)
        return;
    var a = D.assets[_selectedAsset];
    if (!a)
        return;
    if (field === "criticite" || field === "quantite")
        val = parseInt(val) || 1;
    a[field] = val;
    _markManual(a, field);
    _save();
...
function backToAssets() { _selectedAsset = null; renderPanel(); }
window.backToAssets = backToAssets;
function _depSearchInput(handler, listId, excludeId, existingIds) {
    ...
    D.assets.forEach(function (x) {
        if (x.id === excludeId || existingIds.indexOf(x.id) >= 0)
            return;
        h += '<option value="' + esc(x.id) + '">' + esc(x.id + " — " + x.nom + " (" + _typeLabel(x.type) + ")") + '</option>';
    });
    D.groupes.forEach(function (g) {
        if (g.id === excludeId || existingIds.indexOf(g.id) >= 0)
            return;
...
function addAssetDep() {
    if (_selectedAsset === null)
        return;
    var input = document.getElementById("asset-dep-search-input");
    ...
    if (exists && a.depends_on.indexOf(id) < 0)
        a.depends_on.push(id);
    renderPanel();
    _save();
}
window.addAssetDep = addAssetDep;
function removeAssetDep(id) {
    if (_selectedAsset === null)
        return;
    var a = D.assets[_selectedAsset];
...
function removeAssetGroupe(gid) {
    ...
    var g = D.groupes.find(function (x) { return x.id === gid; });
    if (g)
        g.asset_ids = (g.asset_ids || []).filter(function (x) { return x !== a.id; });
    renderPanel();
    _save();
}
...
function renderGroupeList() {
    ...
    D.groupes.forEach(function (g, i) {
        ...
        h += '</div>';
        if (g.principe)
            h += '<div class="ct-text-label ct-journal-sep ct-mt-1 ct-overflow-hidden ct-ellipsis ct-nowrap">' + esc(g.principe) + '</div>';
        h += '<div class="ct-flex ct-gap-3 ct-mt-1 ct-text-label ct-muted">';
        h += '<span>' + n + ' ' + t("nav.assets").toLowerCase() + '</span>';
        var raciOk = g.raci && g.raci.installation && g.raci.installation.r;
...
function openGroupe(idx) { _selectedGroupe = parseInt(String(idx)); _groupeTab = "info"; renderPanel(); }
...
function renderGroupeDetail() {
    ...
    switch (_groupeTab) {
        case "info":
            h += _renderGroupeInfo(g);
            break;
        case "raci":
            h += _renderGroupeRaci(g);
            break;
        case "politiques":
            h += _renderGroupePolitiques(g);
            break;
        case "actifs":
            h += _renderGroupeActifs(g);
            break;
        case "deps":
            h += _renderGroupeDeps(g);
            break;
    }
    return h;
}
...
function _ensureRaciArray(g) {
    if (Array.isArray(g.raci))
        return;
...
function _renderGroupeRaci(g) {
    ...
    g.raci.forEach(function (row, idx) {
...
function saveRaciCell(idx, role, val) {
    if (_selectedGroupe === null)
        return;
    var g = D.groupes[_selectedGroupe];
    _ensureRaciArray(g);
...
function saveRaciActivite(idx, val) {
    if (_selectedGroupe === null)
        return;
    var g = D.groupes[_selectedGroupe];
    _ensureRaciArray(g);
    if (g.raci[idx])
        g.raci[idx].activite = val;
    _save();
}
...
function addRaciRow() {
    ...
    var g = D.groupes[_selectedGroupe];
    _ensureRaciArray(g);
    g.raci.push({ activite: "", r: "", a: "", c: "", i: "" });
    renderPanel();
    _save();
}
window.addRaciRow = addRaciRow;
function removeRaciRow(idx) {
    if (_selectedGroupe === null)
        return;
    var g = D.groupes[_selectedGroupe];
    _ensureRaciArray(g);
...
window._openGroupeAssetPicker = function () {
    if (_selectedGroupe === null || !window.ct_modal)
        return;
    var g = D.groupes[_selectedGroupe];
    ...
    ct_modal.open({
        ...
        buttons: [
            ...
            { id: "add", primary: true, label: t("btn_add") || "Ajouter",
                result: function () {
                    var boxes = Array.from(document.querySelectorAll("#grp-pick-list .grp-pick-cb:checked"));
                    var ids = boxes.map(function (b) { return b.value; });
                    if (!ids.length) {
                        showStatus(t("groupe.pick_assets_empty") || "Aucun actif sélectionné", true);
                        return false;
                    }
                    return ids;
                } }
...
window._groupePickerFilter = function (val) {
    var q = (val || "").toLowerCase().trim();
    var rows = Array.from(document.querySelectorAll("#grp-pick-list .grp-pick-row"));
...
function _linkAssetsToGroup(assetIds, groupId) {
    ...
    assetIds.forEach(function (aid) {
        ...
        if (!a.groupe_ids)
            a.groupe_ids = [];
        if (a.groupe_ids.indexOf(groupId) < 0)
            a.groupe_ids.push(groupId);
        // Count only NEW memberships — assets already in the group (or a
        // duplicated id in the input) don't inflate the "added" total.
        if (g.asset_ids.indexOf(aid) < 0) {
            g.asset_ids.push(aid);
            count++;
        }
    });
    if (count && typeof _save === "function")
        _save();
    return count;
}
function addGroupeAsset(aid) {
    if (!aid || _selectedGroupe === null)
        return;
    _linkAssetsToGroup([aid], D.groupes[_selectedGroupe].id);
    renderPanel();
}
window.addGroupeAsset = addGroupeAsset;
function removeGroupeAsset(aid) {
    if (_selectedGroupe === null)
        return;
    var g = D.groupes[_selectedGroupe];
    ...
    var a = D.assets.find(function (x) { return x.id === aid; });
    if (a)
        a.groupe_ids = (a.groupe_ids || []).filter(function (x) { return x !== g.id; });
    renderPanel();
    _save();
}
window.removeGroupeAsset = removeGroupeAsset;
// ── Groupe dependencies tab ──────────────────────────────────
function _renderGroupeDeps(g) {
    if (!g.depends_on_groups)
        g.depends_on_groups = [];
    var deps = g.depends_on_groups.map(function (id) {
        var grp = D.groupes.find(function (x) { return x.id === id; });
        if (grp)
            return { id: grp.id, nom: grp.nom, badge: _badge(t("nav.groupes"), "var(--ct-ink-2)"), kind: "groupe" };
        var asset = D.assets.find(function (x) { return x.id === id; });
...
function addGroupeDep() {
    if (_selectedGroupe === null)
        return;
    var input = document.getElementById("groupe-dep-search-input");
    var id = input ? input.value.trim().split(" — ")[0].trim() : "";
    if (!id)
        return;
    var g = D.groupes[_selectedGroupe];
    if (!g.depends_on_groups)
        g.depends_on_groups = [];
    var exists = D.assets.some(function (x) { return x.id === id; }) || D.groupes.some(function (x) { return x.id === id; });
    if (exists && id !== g.id && g.depends_on_groups.indexOf(id) < 0)
        g.depends_on_groups.push(id);
    renderPanel();
    _save();
}
window.addGroupeDep = addGroupeDep;
function removeGroupeDep(id) {
    if (_selectedGroupe === null)
        return;
    var g = D.groupes[_selectedGroupe];
...
function saveGroupePol(pol, field, val) {
    if (_selectedGroupe === null)
        return;
    var g = D.groupes[_selectedGroupe];
    if (!g[pol])
...
function saveGroupePolCheck(pol, field, el) {
    if (_selectedGroupe === null)
        return;
    var g = D.groupes[_selectedGroupe];
    if (!g[pol])
        g[pol] = {};
    g[pol][field] = el.checked;
    _save();
}
...
// ═══════════════════════════════════════════════════════════════
function _ctConfirm(title, body, onYes) {
...
function importCsvDialog() {
    var el = document.getElementById("csv-input");
    if (!el)
        return;
    el.value = "";
...
function _importCsvFile(file) {
    if (window.AssetAPI != null && window.getActiveProjectId != null) {
        var pid = getActiveProjectId();
        if (!pid) {
            showStatus(t("csv.no_project"), true);
            return;
        }
        AssetAPI.importCsv(pid, file).then(function (result) {
        ...
        reader.onload = function (e) {
            _parseCsvLocal(e.target.result);
        };
        reader.readAsText(file);
    }
}
function _parseCsvLocal(text) {
    var lines = text.replace(/\r\n/g, "\n").replace(/\r/g, "\n").split("\n");
    if (lines.length < 2) {
        showStatus(t("csv.error"), true);
        return;
    }
    ...
    for (var i = 1; i < lines.length; i++) {
        ...
        for (var j = 0; j < headers.length; j++) {
            ...
            if (field && vals[j])
                mapped[field] = vals[j].trim();
        }
        if (!mapped.nom)
            continue;
        var assetType = (mapped.type || "application").toLowerCase().replace(/[ -]/g, "_");
        if (validTypes.indexOf(assetType) < 0)
            assetType = typeAliases[assetType] || "application";
...
async function _aiPost(feature, payload) {
    var resp = await fetch("api/ai/asset/" + feature, {
        method: "POST",
        credentials: "same-origin",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(Object.assign({}, _aiMeta(), payload))
    });
    if (!resp.ok) {
...
function aiSuggestDescription() {
    if (_selectedAsset === null || !(window._aiIsEnabled != null && _aiIsEnabled()))
        return;
    var a = D.assets[_selectedAsset];
    ...
    _aiPost("suggest-description", {
        ...
        a.description = text;
        var el = document.getElementById("asset-description");
        if (el)
            el.value = text;
        _save();
...
function aiSuggestPrincipe() {
    if (_selectedGroupe === null || !(window._aiIsEnabled != null && _aiIsEnabled()))
        return;
    var g = D.groupes[_selectedGroupe];
    var members = (g.asset_ids || []).map(function (id) { return D.assets.find(function (x) { return x.id === id; }); }).filter(Boolean);
    _aiShowLoading(t("ai.loading"));
    _aiPost("suggest-principe", {
        ...
        var text = (resp.text || "").trim();
        g.principe = text;
        var el = document.getElementById("groupe-principe");
        if (el)
            el.value = text;
        _save();
        showStatus(t("ai.done"));
...
function aiSuggestRaci() {
    ...
    _aiPost("suggest-raci", {
        ...
        principe: g.principe || "",
        criticite_label: _critLabel(g.criticite || 1),
        existing_activities: g.raci.map(function (r) { return r.activite; })
    }).then(function (resp) {
        _aiClosePanel();
...
function aiSuggestPolitiques() {
    ...
    _aiPost("suggest-policies", {
        ...
        _aiClosePanel();
        if (resp) {
            if (resp.politique_sauvegarde)
                g.politique_sauvegarde = Object.assign(g.politique_sauvegarde || {}, resp.politique_sauvegarde);
            if (resp.politique_supervision)
                g.politique_supervision = Object.assign(g.politique_supervision || {}, resp.politique_supervision);
            if (resp.politique_maj)
                g.politique_maj = Object.assign(g.politique_maj || {}, resp.politique_maj);
            renderPanel();
            _save();
            showStatus(t("ai.done"));
        }
...
window.showPluginModal = function (pluginId) {
    var pid = window.getActiveProjectId != null ? getActiveProjectId() : null;
    if (!pid) {
        showStatus("No active project", true);
        return;
    }
    if (!window.ct_modal) {
        showStatus("ct_modal not loaded", true);
    ...
    function _buildBody(currentType) {
...
window._aplgTypeChanged = function (val) {
    ...
    if (container)
        container.innerHTML = _renderPluginConfigFields(tdef, {});
};
function _renderPluginConfigFields(tdef, values) {
    if (!tdef || !Array.isArray(tdef.config_schema))
        return "";
    values = values || {};
...
function _collectPluginForm() {
    var typeEl = document.getElementById("aplg-type");
    var labelEl = document.getElementById("aplg-label");
    var scheduleEl = document.getElementById("aplg-schedule");
    var enabledEl = document.getElementById("aplg-enabled");
    var priorityEl = document.getElementById("aplg-priority");
    if (!typeEl) {
        showStatus("Form missing", true);
        return null;
    }
    var pluginType = (typeEl.value || "").trim();
    if (!pluginType) {
        showStatus("Type de connecteur manquant", true);
        return null;
    }
    var config = {};
    document.querySelectorAll("[data-aplg-key]").forEach(function (el) {
    ...
    if (priorityEl) {
        ...
        if (!isNaN(n))
            prio = Math.max(0, Math.min(1000, n));
    }
    var idEl = document.getElementById("aplg-id");
    return {
...
function _testCurrentForm() {
    ...
    AssetAPI.testPluginConfig(pid, data).then(function (r) {
        if (r.ok) {
...
window.testAssetPlugin = function (pluginId) {
    var pid = window.getActiveProjectId != null ? getActiveProjectId() : null;
    if (!pid)
        return;
    showStatus(t("plg.testing") || "Test en cours...");
    AssetAPI.testPlugin(pid, pluginId).then(function (r) {
        if (r.ok)
            showStatus((t("plg.test_ok") || "Connexion OK") + (r.details ? " — " + r.details : ""));
        else
            showStatus((t("plg.test_fail") || "Échec : ") + (r.error || ""), true);
...
window.syncAssetPlugin = function (pluginId) {
    var pid = window.getActiveProjectId != null ? getActiveProjectId() : null;
    if (!pid)
        return;
    ...
    if (!confirm(msg))
        return;
    if (window._cancelAutosave)
        window._cancelAutosave();
    showStatus(t("plg.syncing") || "Synchronisation en cours...");
    AssetAPI.syncPlugin(pid, pluginId).then(function (r) {
        ...
        AssetAPI.get(pid).then(function (prj) {
...
window.refreshConnectors = function () {
    ...
    function run() {
        ...
        if (window._cancelAutosave)
            window._cancelAutosave();
        showStatus(t("plg.syncing") || "Synchronisation en cours...");
        var agg = { found: 0, created: 0, updated: 0, unchanged: 0, retired: 0, reactivated: 0, errors: 0 };
        var i = 0;
        function next() {
            if (i >= enabled.length) {
                finish();
            ...
            var p = enabled[i++];
            AssetAPI.syncPlugin(pid, p.id).then(function (r) {
        ...
        function finish() {
            ...
            var extra = "";
            if (agg.retired)
                extra += " (" + agg.retired + " retiré(s) — absent(s) des connecteurs)";
            if (agg.reactivated)
                extra += " (" + agg.reactivated + " réactivé(s))";
            showStatus(tpl
...
window.deleteAssetPlugin = function (pluginId) {
    var pid = window.getActiveProjectId != null ? getActiveProjectId() : null;
    if (!pid)
        return;
    var p = _pluginList.find(function (x) { return x.id === pluginId; });
    var name = (p && p.label) || pluginId;
    if (!confirm((t("plg.confirm_delete") || "Supprimer le connecteur") + " \"" + name + "\" ?"))
        return;
...
window.showAssetPluginHistory = function (pluginId) {
    var pid = window.getActiveProjectId != null ? getActiveProjectId() : null;
    if (!pid || !window.ct_modal)
        return;
...
function renderMeasuresPanel(c) {
    if (!_measuresLoaded) {
...
function _measureModalBaseOpts() {
    return {
        hideFields: ["type"],
...
function _reloadMeasures() { _measuresLoaded = false; selectPanel("measures"); }
function _newAssetMeasure() {
    var pid = window.getActiveProjectId != null ? getActiveProjectId() : null;
    if (!pid || !window.ct_measure_modal)
        return;
    var opts = _measureModalBaseOpts();
    opts.title = t("measure.new") || "Nouvelle action";
    window.ct_measure_modal.open(null, opts).then(function (result) {
        if (!result)
            return;
        AssetAPI.createMeasure(pid, result).then(function () {
            _reloadMeasures();
            if (typeof showStatus === "function")
                showStatus(t("measure.created") || "Mesure créée");
        }).catch(function (e) {
            if (typeof showStatus === "function")
                showStatus(e.message || String(e), true);
        });
...
function _editAssetMeasureRow(id) {
    var pid = window.getActiveProjectId != null ? getActiveProjectId() : null;
    var m = _measureList.find(function (x) { return x.id === id; });
    if (!pid || !m || !window.ct_measure_modal)
        return;
    var opts = _measureModalBaseOpts();
    opts.title = m.id;
    opts.onAddNote = function (_entry, fullLog) {
        m.progress_log = fullLog;
        return AssetAPI.patchMeasure(pid, m.id, { progress_log: fullLog });
    };
    opts.onDelete = function () {
        if (!confirm(t("measure.confirm_delete") || "Supprimer cette action ?"))
            return;
    ...
    window.ct_measure_modal.open(m, opts).then(function (result) {
...
if (typeof window.ct_handleMeasureDeepLink === "function") {
    window.ct_handleMeasureDeepLink({ open: function (mid) {
            ...
            if (_panel !== "measures" && typeof selectPanel === "function")
                selectPanel("measures");
            if (!_measureList.some(function (m) { return m.id === mid; }))
                return false;
