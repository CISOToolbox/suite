...
(function () {
    ...
    window.AssetAPI = {
        ...
        importCsv: function (projectId, file) {
            var formData = new FormData();
            formData.append("file", file);
        ...
        createPlugin: function (pid, body) { return _fetch("/projects/" + pid + "/plugins", { method: "POST", body: body }); },
        patchPlugin: function (pid, id, body) { return _fetch("/projects/" + pid + "/plugins/" + id, { method: "PATCH", body: body }); },
        ...
        syncPlugin: function (pid, id) { return _fetch("/projects/" + pid + "/plugins/" + id + "/sync", { method: "POST" }); },
    ...
    window._appInitCallback = function () {
        function _loadAndRender(id) {
            AssetAPI.get(id).then(function (project) {
                _activeId = project.id;
                _serverRev = project.server_rev || 0;
                localStorage.setItem("asset_active_project", _activeId);
        ...
        function _createAndRender() {
            ...
            AssetAPI.create({ name: "", data: initData }).then(function (project) {
                _activeId = project.id;
                _serverRev = project.server_rev || 0;
                localStorage.setItem("asset_active_project", _activeId);
                Object.keys(D).forEach(function (k) { delete D[k]; });
        ...
        AssetAPI.list().then(function (items) {
            if (items.length > 0) {
    ...
    document.addEventListener("visibilitychange", function () {
        if (document.visibilityState !== "visible" || !_activeId || _saveTimer)
            return;
        AssetAPI.get(_activeId).then(function (project) {
