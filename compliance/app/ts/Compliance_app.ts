...
function _genMesureId() {
    ...
    while (D.mesures.some(m => m.id === "M-" + String(_nextMesureId).padStart(3,"0")
            || m.id === "MES-" + String(_nextMesureId).padStart(3,"0"))) _nextMesureId++;
...
var _derogByKey: Record<string, CtDerRecord> = {};
var _derogLoaded = false;
function _loadDerogations(then?: () => void) {
...
function _internalFwMeta(): ComplianceFwMeta {
    return { label: t("comp.nc.internal_fw"), description: t("comp.nc.internal_fw"), color: "#78716c", custom: true } as ComplianceFwMeta;
}
...
function _getAllFrameworks(): Record<string, ComplianceFwMeta> {
    ...
    if (D.referentiels_actifs && D.referentiels_actifs.indexOf(_INTERNAL_FW) >= 0 && !all[_INTERNAL_FW]) all[_INTERNAL_FW] = _internalFwMeta();
...
function renderAll() {
    ...
    else if (_currentPanel === "nonconformities") renderNonconformities();
...
function _demanderDesactivationOuSuppression(fwId: string, label: string) {
    if (!window.ct_modal) { _desactiverReferentiel(fwId); return; }
    ...
    }).then(function(choix: unknown) {
        if (choix === "off") _desactiverReferentiel(fwId);
        else if (choix === "delete") supprimerReferentiel(fwId);
    });
}

function _desactiverReferentiel(fwId: string) {
    _saveState();
    const pos = D.referentiels_actifs.indexOf(fwId);
    if (pos >= 0) D.referentiels_actifs.splice(pos, 1);
    if (_currentFw === fwId) selectPanel("context");
    renderContext(); renderSidebar(); _autoSave();
}
 ...
 */
function supprimerReferentiel(fwId: string) {
    const meta = _getAllFrameworks()[fwId];
    if (!meta || !meta.custom || fwId === _INTERNAL_FW) return;
    const api = window.ComplianceAPI;
    if (!api || !api.deleteFramework) { showStatus(t("comp.fw.delete_unavailable"), true); return; }
    api.deleteFramework(fwId).then(function() {
        _saveState();
        const pos = D.referentiels_actifs.indexOf(fwId);
        if (pos >= 0) D.referentiels_actifs.splice(pos, 1);
        delete D.referentiels[fwId];
        if (D._custom_frameworks) delete D._custom_frameworks[fwId];
        delete REFERENTIELS_META[fwId];
        if (window._REFERENTIELS_CATALOG) delete window._REFERENTIELS_CATALOG[fwId];
        if (window.COMPLIANCE_REF) delete window.COMPLIANCE_REF[fwId];
        if (_currentFw === fwId) selectPanel("context");
        renderContext(); renderSidebar();
...
function _parseAndImportCSV(csvText: string, filename: string) {
    ...
    _declareFramework({ id: fwId, label: label!, color: color, description: desc,
        ...
        .then(function(ok) {
            if (!ok) { showStatus(t("comp.csv.error_save"), true); return; }
...
function _declareFramework(fw: { id: string; label: string; color: string; description: string;
                                 requirements: ComplianceRefMeasure[] }): Promise<boolean> {
    var api = window.ComplianceAPI;
    if (!api || !api.createFramework) return Promise.resolve(true);
    return api.createFramework(fw as unknown as Record<string, unknown>)
        .then(function() { return true; })
        // 409 = already declared: the outcome we wanted, not a failure.
        .catch(function(e: any) { return String((e && e.message) || "").indexOf("409") >= 0; });
}

function _activateImportedFramework(fwId: string, label: string, color: string,
                                    desc: string, measures: ComplianceRefMeasure[]) {
    // Register in catalog
...
function renderDashboard() {
    if (!_derogLoaded) { _loadDerogations(renderDashboard); return; }
    let h = "";
...
function _renderFwExigences(fwId: string, label: string) {
    if (!_derogLoaded) { _loadDerogations(function() { if (_currentFw === fwId && _currentSubview === "exigences") _renderFwExigences(fwId, label); }); }
...
function _ncOptions(): CtNcOptions {
    return {
        ...
        onChange: function() { _loadDerogations(function() { if (_currentPanel.startsWith("fw:") && _currentFw) _renderFwView(_currentFw, _currentSubview); }); }
    };
}
...
function _createMesureForNc(prefill: { title: string; description: string }): Promise<CtNcItemOption | null> {
    ...
    return window.ct_measure_modal.open({ description: prefill.title, details: prefill.description }, {
        ...
        var payload = {
            ...
            date_cible: result.date_cible || "", recurrence: "", dernier_controle: "", preuves_ids: [] as string[]
        };
        var done = function(created: ComplianceMesure): CtNcItemOption {
        ...
        if (window.ComplianceAPI && typeof window._getActiveProjectId === "function") {
            return window.ComplianceAPI.createMeasure(window._getActiveProjectId(), payload).then(done)
                .catch(function(e: any) { showStatus("Erreur création : " + (e.message || e), true); return null; });
        }
        var created = done(payload as ComplianceMesure); _autoSave(); return created;
    });
}
 ...
 *  assessment. */
function _syncInternalFramework(): void {
    var api = window.ComplianceAPI;
    if (!api || !api.createFramework) return;
    ...
    _declareFramework({ id: _INTERNAL_FW, label: t("comp.nc.internal_fw"), color: "#78716c",
                        description: t("comp.nc.internal_fw"), requirements: defs as ComplianceRefMeasure[] })
        .then(function(ok) {
            if (ok && api.putFrameworkRequirements) {
                api.putFrameworkRequirements(_INTERNAL_FW, defs).catch(function() { /* label kept, definition stale */ });
            }
        });
}
...
function _createControlForNc(nc: { title: string; description: string; domain: string }): Promise<CtNcItemOption | null> {
    ...
    return window.ct_modal.open({ title: t("comp.nc.create_control"), body: h, size: "md", buttons: [
        ...
        var record = function(controlId: string | null): CtNcItemOption {
            _saveState();
            ...
            if (controlId) entry.id = controlId;
        ...
        if (window.ComplianceAPI && pid) {
            return window.ComplianceAPI.createControl(pid, { framework_id: _INTERNAL_FW, ref: res.ref, thematique: res.theme, mesure: res.mesure })
                .then(function(created: any) { return record(created && created.id != null ? String(created.id) : null); })
                .catch(function(e: any) { showStatus(t("comp.nc.create_control_failed", { msg: e && e.message ? e.message : String(e) }), true); return null; });
        }
        return record(null);
    });
}
...
window._editMesureRow = function(row: { id: string; __fwId?: string | null; __fwEnsured?: boolean }) {
...
function _showPreuveModal() {
    ...
    var _row = function(label: string, inner: string) {
    ...
    h += _row(t("comp.prv.placeholder_label"),
        '<input type="text" class="ct-w-full" value="' + esc(p.label||"") + '" data-change="_updatePreuveField" data-args=\'' + _da(p.id,"label") + '\' data-pass-value />');
    h += _row(t("comp.prv.placeholder_url"),
        '<input type="text" class="ct-w-full" value="' + esc(p.url||"") + '" data-change="_updatePreuveField" data-args=\'' + _da(p.id,"url") + '\' data-pass-value />');
    h += '<div class="ct-grid ct-grid-2 ct-gap-3">';
    h += _row(t("comp.prv.label_kind"),
        '<select class="ct-w-full" data-change="_updatePreuveField" data-args=\'' + _da(p.id,"kind") + '\' data-pass-value>'
    ...
    h += '</div>';
    h += '<div class="ct-grid ct-grid-2 ct-gap-3">';
    h += _row(t("comp.prv.label_obtention"),
        '<input type="date" class="ct-w-full" value="' + esc(p.date_obtention||"") + '" data-change="_updatePreuveField" data-args=\'' + _da(p.id,"date_obtention") + '\' data-pass-value />');
    h += _row(t("comp.prv.label_expiration"),
        '<input type="date" class="ct-w-full" value="' + esc(p.date_expiration||"") + '" data-change="_updatePreuveField" data-args=\'' + _da(p.id,"date_expiration") + '\' data-pass-value />');
    h += '</div>';
    h += _row(t("comp.prv.placeholder_comment"),
        '<textarea rows="2" class="ct-w-full" data-change="_updatePreuveField" data-args=\'' + _da(p.id,"commentaire") + '\' data-pass-value data-input="_autoHeight" data-pass-el>' + esc(p.commentaire||"") + '</textarea>');
    h += '</div>';
    ...
    var _up = (window as unknown as { ct_userpicker?: any }).ct_userpicker;
    if (_up && _up.mount) {
        _up.mount({
            slotId: "preuve-owner-slot", pickerId: "preuve-owner",
            value: String((p as unknown as Record<string, unknown>).owner || ""),
