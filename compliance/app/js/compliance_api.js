...
(function () {
    ...
    window.ComplianceAPI = {
        ...
        deleteControl: function (pid, cid) { return _fetch("/projects/" + pid + "/controls/" + cid, { method: "DELETE" }); },
        patchMeasure: function (pid, mid, f) { return _fetch("/projects/" + pid + "/measures/" + mid, { method: "PATCH", body: f }); },
        createMeasure: function (pid, d) { return _fetch("/projects/" + pid + "/measures", { method: "POST", body: d }); },
        deleteMeasure: function (pid, mid) { return _fetch("/projects/" + pid + "/measures/" + mid, { method: "DELETE" }); },
        patchProof: function (pid, rid, f) { return _fetch("/projects/" + pid + "/proofs/" + rid, { method: "PATCH", body: f }); },
    ...
    window._persistCreate = function (entityType, data) {
        ...
        if (fn)
            fn(data).catch(function (e) { console.error("POST " + entityType + " failed:", e); });
        else
            _autoSave();
    };
    ...
    document.addEventListener("visibilitychange", function () {
        if (document.visibilityState !== "visible" || !_activeId || !_dataReady)
            return;
        if (Object.keys(_dirty).length || _saveTimer)
            return;
        ComplianceAPI.get(_activeId).then(function (project) {
            if ((project.server_rev || 0) === _serverRev)
                return;
    ...
    window._appInitCallback = function () {
        function _loadAndRender(id) {
            ComplianceAPI.get(id).then(function (project) {
                ...
                Object.keys(D).forEach(function (k) { delete D[k]; });
                Object.assign(D, pdata);
                if (typeof window._setDataReady === "function")
                    window._setDataReady();
        ...
        function _handleMeasureDeepLink() {
            ...
            window.ct_handleMeasureDeepLink({ open: function (mid) {
                    var mesures = (D && D.mesures) || [];
                    if (!mesures.some(function (m) { return m.id === mid; }))
                        return false;
                    if (typeof selectPanel === "function")
                        selectPanel("plan");
                    if (typeof window._editMesureRow === "function")
                        window._editMesureRow({ id: mid });
                    return true;
                } });
        }
    ...
    (function () {
        var _fwCache = {};
        var _g = window;
        window._ensureFramework = function (fwId, cb) {
            if (_fwCache[fwId]) {
                if (_g.REFERENTIELS_META)
                    _g.REFERENTIELS_META[fwId] = _fwCache[fwId];
                if (window.COMPLIANCE_REF)
                    window.COMPLIANCE_REF[fwId] = _fwCache[fwId];
                cb();
                return;
            }
        ...
        fetch(BASE + "/frameworks").then(function (r) { return r.json(); }).then(function (list) {
            ...
            if (typeof _g.renderSidebar === "function" && _g.D && _g.D.meta)
                _g.renderSidebar();
        }).catch(function (e) { console.warn("Failed to load framework catalog from API:", e); });
    })();
})();
