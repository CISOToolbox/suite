 ...
 */
var ACCESS_INIT_DATA = { si_users: [], applications: [], reviews: [], measures: [], service_accounts: [], metadata: { organization: "", created: "" } };
window.CT_CONFIG = {
...
window.SCHEMA_REV = 1;
var D = JSON.parse(JSON.stringify(ACCESS_INIT_DATA));
var _panel = "dashboard";
var _selectedUser = null, _selectedApp = null, _selectedReview = null, _selectedSA = null;
...
function _initDataAndRender(cb) {
    ...
    _panel = location.hash === "#nonconformities" ? "nonconformities" : "dashboard";
    _selectedUser = _selectedApp = _selectedReview = _selectedSA = null;
    renderAll();
    _loadPlugins(function () { _updateSidebarAccordion(_panel); renderPanel(); });
    _installLiveReload();
    if (cb)
        cb();
}
...
function _reloadProjectData(onAfter) {
...
function _installLiveReload() {
    if (_liveReloadInstalled)
        return;
...
// NAVIGATION
// ═══════════════════════════════════════════════════════════════
function selectPanel(id) { _panel = id; _selectedUser = _selectedApp = _selectedReview = _selectedSA = null; document.querySelector(".ct-rail, .sidebar")?.classList.remove("open"); _updateSidebarAccordion(id); renderPanel(); }
function renderPanel() {
    ...
    switch (_panel) {
        case "dashboard":
            ...
            break;
        case "users":
            c.innerHTML = _selectedUser !== null ? renderUserDetail() : renderUserList();
            break;
        case "apps":
            c.innerHTML = _selectedApp !== null ? renderAppDetail() : renderAppList();
            break;
        case "reviews":
            c.innerHTML = _selectedReview !== null ? renderReviewDetail() : renderReviewList();
            break;
        case "service_accounts":
            c.innerHTML = _selectedSA !== null ? renderSADetail() : renderSAList();
            break;
        case "measures":
            c.innerHTML = renderMeasureList();
            break;
        case "plugins":
            c.innerHTML = renderPlugins();
            break;
...
function _updateSidebarAccordion(id) { document.querySelectorAll(".ct-rail-item").forEach(function (el) { var a = el.getAttribute("data-args"); if (a && a.indexOf('"' + id + '"') >= 0)
...
function _sensibilisationCard(u) {
    var hist = (u && u.sensibilisation_history) || {};
    var camps = Object.keys(hist);
    if (camps.length === 0) {
        return _proofCard("sensibilisation", t("user.sensibilisation"), u.sensibilisation, u.sensibilisation_date, u.sensibilisation_justification, "sensibilisation", "sensibilisation_date", "sensibilisation_justification");
    }
...
function _accessTone(v) {
    return _ACCESS_TONES[(v || "").toString()] || "neutral";
}
// IdP account active/disabled badge. undefined/null = unknown (connector didn't report).
function _accountTag(enabled) {
    if (enabled === true)
        return '<span class="ct-badge" data-tone="low">' + (t("user.account_active") || "Actif") + '</span>';
    if (enabled === false)
        return '<span class="ct-badge" data-tone="critical">' + (t("user.account_disabled") || "Désactivé") + '</span>';
...
function _findApp(id) { return D.applications.find(function (a) { return a.id === id; }); }
...
function renderDashboard() {
    ...
    var derogated = 0;
    D.reviews.forEach(function (r) { (r.entries || []).forEach(function (e) { if (e.decision === "derogated")
        derogated++; }); });
    ...
    if (D.si_users.length) {
        ...
        D.si_users.forEach(function (u) {
            if (u.account_enabled === true)
                accOn++;
            else if (u.account_enabled === false)
                accOff++;
            else
                accUnk++;
        });
        ...
        h += window._svgDonut({ center_label: D.si_users.length, segments: [
                ...
                { label: t("dashboard.acct_unknown"), value: accUnk, color: "dark" }
            ] }, { size: 124, thickness: 18 });
        h += '</div>';
    }
    if (D.applications.length) {
        // Known types first, then any legacy value so the centre total always
        // equals the perimeter count.
        var byType = {};
        ...
        h += window._svgBar({ buckets: [
                ...
                { label: t("dashboard.closed_reviews"), value: closed, color: "green" }
            ] }, { width: 280, labelWidth: 130 });
        h += '</div>';
    }
    var saAll = D.service_accounts || [];
    if (saAll.length) {
        // A single account can be both rotation-overdue and expiring: the two
        // rows count independently (same figures as the KPIs); "ok" is the
        // complement of their union.
        var saOk = saAll.filter(function (sa) {
        ...
        h += window._svgBar({ scale: saAll.length, buckets: [
                ...
                { label: t("dashboard.sa_expiring_short"), value: _countExpiringSoon(), color: "orange" }
            ] }, { width: 280, labelWidth: 130 });
        h += '</div>';
    }
    h += '</div>';
    ...
    h += ct_table.render({
        ...
        columns: [
            { key: "id", label: "ID", width: "90px",
                render: function (u) { return '<span class="ct-text-label ct-muted">' + esc(u.id) + '</span>'; } },
            { key: "nom", label: t("user.nom"),
                render: function (u) { return '<strong>' + esc(u.nom) + '</strong>'; } },
            { key: "prenom", label: t("user.prenom"),
                render: function (u) { return esc(u.prenom); } },
            { key: "email", label: "Email",
                render: function (u) { return '<span>' + esc(u.email) + '</span>'; } },
            { key: "statut", label: t("user.statut_label"), width: "130px",
                render: function (u) { return esc(_statusLabel(u.statut)); } },
            { key: "fonction", label: t("user.fonction"),
                render: function (u) { return '<span>' + esc(u.fonction) + '</span>'; } },
            ...
            { key: "compliance", label: t("user.compliance"), width: "120px",
                render: function (u) { return _compTag(u.politique_validee) + ' ' + _compTag(u.mfa_active) + ' ' + _compTag(u.sensibilisation); } }
        ]
    });
    setTimeout(function () {
...
window.syncUsersFromPilot = function () {
    ...
    _ctConfirm(t("user.sync_pilot_confirm_title") || "Synchroniser depuis Pilot", t("user.sync_pilot_confirm_body") || "Les utilisateurs de l'annuaire Pilot seront importes ou mis a jour (nom, prenom, fonction). Les flags de conformite existants ne sont pas ecrases. Continuer ?", function () {
...
function addUser() {
    ...
    _selectedUser = D.si_users.length - 1;
    renderPanel();
    _save();
}
...
function renderUserDetail() {
    ...
    h += _proofCard("politique", t("user.politique_validee"), u.politique_validee, u.politique_date, u.politique_justification, "politique_validee", "politique_date", "politique_justification");
    h += _proofCard("mfa", t("user.mfa_active"), u.mfa_active, u.mfa_date, u.mfa_justification, "mfa_active", "mfa_date", "mfa_justification");
    h += _sensibilisationCard(u);
    h += _proofCard("background_check", t("user.background_check") || "Background check", u.background_check, u.background_check_date, u.background_check_justification, "background_check", "background_check_date", "background_check_justification");
    // NDA — mandatory for prestataires, displayed only then to keep the UI tidy.
    if ((u.type_compte || "salarie") === "prestataire") {
        h += _proofCard("nda", t("user.nda") || "NDA signé", u.nda_signed, u.nda_date, u.nda_justification, "nda_signed", "nda_date", "nda_justification");
    }
    h += '</div>';
    // Legacy background-check URL — kept editable when present, hidden otherwise.
    if (u.background_check_url) {
        h += '<div>';
        h += _field("background_check_url", (t("user.background_check_url") || "Background check (URL legacy)"), "url", u.background_check_url);
        h += '</div>';
    }
    h += '</div>';
...
function _mountManagerPicker() {
    var up = window.ct_userpicker;
    if (!up || _selectedUser === null)
        return;
    var uu = D.si_users[_selectedUser];
    if (!uu)
        return;
    ...
    up.mount({
        ...
        ["access-user-manager-search", "access-user-manager-plain"].forEach(function (eid) {
            ...
            if (el) {
                el.addEventListener("blur", _commitManager);
                el.addEventListener("change", _commitManager);
            }
        });
        var wrap = document.getElementById("access-user-manager-wrap");
        if (wrap)
            wrap.addEventListener("click", function () { setTimeout(_commitManager, 0); });
    });
}
...
function _commitManager() {
    if (_selectedUser === null || !_mgrHandle)
        return;
    var target = D.si_users[_selectedUser];
    if (!target)
        return;
...
function saveUserCheck(field, el) {
    if (_selectedUser === null)
        return;
    D.si_users[_selectedUser][field] = el.checked;
    _save();
}
...
document.addEventListener("click", function (e) {
    ...
    if (dd && !e.target.closest("#ent-perim-dd, #ent-perim-search"))
        dd.hidden = true;
});
...
function _canEditEntitlements(target) {
    var cu = window._currentUser;
    if (!cu)
        return true; // no auth = full access
    if (window._moduleRole === "admin" || cu.role === "admin")
        return true;
    var actor = String(cu.email || "").trim().toLowerCase();
    if (!actor)
        return false;
    var byEmail = {};
    D.si_users.forEach(function (x) { if (x.email)
        byEmail[x.email.toLowerCase()] = x; });
    var visited = {};
    var cur = target;
    for (var i = 0; i < 10; i++) {
        var mgr = cur ? String(cur.manager_email || "").trim().toLowerCase() : "";
        if (!mgr || visited[mgr])
            break;
        if (mgr === actor)
            return true;
...
function _loadEntitlements() {
    ...
    var pid = window.getActiveProjectId ? getActiveProjectId() : null;
    if (!pid || !window.AccessAPI) {
        var s0 = document.getElementById("user-entitlements");
        if (s0)
            s0.innerHTML = "";
        return;
    }
    ...
    Promise.all([AccessAPI.listEntitlements(pid, uid), AccessAPI.listEntitlementAudit(pid, uid)]).then(function (res) {
        if (_selectedUser === null || D.si_users[_selectedUser].id !== uid)
...
function _renderEntitlements() {
    ...
    if (canEdit) {
        ...
        if (!D.applications.length)
            h += '<div class="ct-py-1 ct-px-2 ct-muted ct-text-label">' + esc(t("app.empty") || "Aucun périmètre") + '</div>';
        h += '</div></div>';
...
window.addEntitlement = function () {
    if (_selectedUser === null || !_entDraftPerim)
        return;
    var u = D.si_users[_selectedUser];
    if (!u)
        return;
    ...
    if (!pid || !window.AccessAPI)
        return;
    var roles = [];
    document.querySelectorAll("#ent-roles-dd input:checked").forEach(function (el) { roles.push(el.value); });
    if (!roles.length)
        roles = [""]; // perimeter-level entitlement (no specific role)
...
function _proofCard(id, label, _checked, dateVal, justif, fieldBool, fieldDate, fieldJustif) {
...
function renderAppList() {
    ...
    h += ct_table.render({
        ...
        columns: [
            { key: "id", label: "ID", width: "90px",
                render: function (a) { return '<span class="ct-text-label ct-muted">' + esc(a.id) + '</span>'; } },
            { key: "nom", label: t("app.nom"),
                render: function (a) { return '<strong>' + esc(a.nom) + '</strong>'; } },
            { key: "url", label: "URL",
                render: function (a) { return '<span>' + esc(a.url || "-") + '</span>'; } },
            { key: "reviewers", label: t("app.reviewers"),
                render: function (a) { return '<span>' + esc(a.__reviewer_names || "-") + '</span>'; } },
            { key: "frequence_revue", label: t("app.frequence"), width: "130px",
                render: function (a) { return '<span data-tone="info">' + esc(_freqLabel(a.frequence_revue)) + '</span>'; } },
            { key: "last_review", label: t("app.last_review"), width: "130px",
                render: function (a) { return '<span>' + esc(a.__last_review || "-") + '</span>'; } }
        ]
    });
    setTimeout(function () {
        if (!window.ct_bulkbar)
...
function backToApps() { _selectedApp = null; renderPanel(); }
...
function renderAppDetail() {
    ...
    if (window._dirGetSource && _dirGetSource() === "pilot") {
        ...
        if (currentReviewers.length) {
            ...
            currentReviewers.forEach(function (rid) {
                ...
                if (su && su.email)
                    h += '<span class="app-reviewer-email">' + esc(su.email) + '</span>';
                h += '</span>';
        ...
        D.si_users.forEach(function (u) {
            if (currentReviewers.indexOf(u.id) >= 0)
                return;
    ...
    if (!appReviews.length) {
        ...
        if (activeReviews.length) {
            ...
            activeReviews.forEach(function (r) {
                ...
                if (total)
                    h += '<div class="progress-bar ct-mt-1"><div class="progress-bar-fill" style="width:' + pct + '%"></div></div>';
                h += '</div>';
...
function addAppReviewer(uid) {
    if (!uid || _selectedApp === null)
        return;
    var a = D.applications[_selectedApp];
    if (!a.reviewers)
        a.reviewers = [];
    if (a.reviewers.indexOf(uid) < 0)
        a.reviewers.push(uid);
    renderPanel();
    _save();
}
window.addAppReviewer = addAppReviewer;
function removeAppReviewer(uid) {
    if (_selectedApp === null)
        return;
    var a = D.applications[_selectedApp];
...
function renderReviewList() {
    ...
    if (!_reviewListFilter)
        _reviewListFilter = { en_cours: true, a_demarrer: true, closed: false };
    ...
    if (f.en_cours && active.length) {
        ...
        active.forEach(function (r, idx) {
            ...
            if (total) {
                h += '<div class="progress-bar"><div class="progress-bar-fill" style="width:' + Math.round(decided / total * 100) + '%"></div></div>';
            }
            h += '</div>';
        });
    }
...
function backToReviews() {
    ...
    _reviewFilters = null;
    if (_reviewReturn) {
        _selectedApp = _reviewReturn.app;
        _reviewReturn = null;
        _panel = "apps";
        _updateSidebarAccordion("apps");
    }
    renderPanel();
}
...
function startReview(appId) {
    ...
    if (pid && window.AccessAPI) {
        AccessAPI.createReview(pid, { application_id: appId }).then(function (rev) {
            D.reviews.push(rev);
            _panel = "reviews";
            _selectedReview = D.reviews.length - 1;
            renderPanel();
        }).catch(function (e) { showStatus(e.message, true); });
    }
    else {
...
function _hasAdminRights(e) {
    var blob = (e.roles || "") + " " + (e.groups || "");
    return _ADMIN_KEYWORDS.test(blob);
}
function _highlightAdminTerms(text) {
    if (!text)
        return "-";
...
function _findServiceAccount(identifier) {
    if (!identifier)
        return null;
...
function _getEntryWarnings(e, su) {
    var warnings = [];
...
function renderReviewDetail() {
    var r = D.reviews[_selectedReview];
    if (!r)
    ...
    if (isClosed)
        h += '<span>' + t("review.closed_at") + ': ' + esc(r.closed_at) + ' (' + esc(r.closed_by) + ')</span>';
    h += '<span class="count">' + decided + '/' + entries.length + ' ' + t("review.decided") + '</span>';
    if (nc)
        h += '<span class="ct-text-critical count">' + nc + ' NC</span>';
    if (warnedCount)
    ...
    var enriched = entries.map(function (e, idx) {
        ...
        if (lastLogin) {
            ...
            if (!isNaN(d.getTime()))
                daysSince = Math.floor((now - d.getTime()) / 86400000);
        }
        return {
    ...
    if (!_reviewFilters)
        _reviewFilters = { admin: false, stale90: false, never_logged: false, alerts: false, pending: false };
    // Filter pills — toggle via data-click
    var pill = function (key, label, count, color) {
    ...
    if (!isClosed)
        h += pill("pending", t("review.filter_pending") || "En attente de decision", nPending, "var(--ct-ink-2)");
    ...
    var visible = enriched.filter(function (x) {
        if (_reviewFilters.admin && !x.isAdmin)
            return false;
        if (_reviewFilters.stale90 && !(x.daysSince !== null && x.daysSince > 90))
            return false;
        if (_reviewFilters.never_logged && x.daysSince !== null)
            return false;
        if (_reviewFilters.alerts && !x.warnings.length)
            return false;
        if (_reviewFilters.pending && x.entry.decision !== "pending")
            return false;
        return true;
    });
    if (!visible.length) {
    ...
    visible.forEach(function (x) {
        ...
        if (isClosed) {
            ...
            if (e.decision === "non_conforme")
                h += ' <button type="button" class="ct-btn" data-size="xs" data-write data-click="_requestEntryDerogation" data-args=\'' + _da(x.idx) + '\'>' + esc(t("review.derogation_btn")) + '</button>';
        }
...
function setDecision(idx, decision) {
    if (_selectedReview === null)
        return;
    var r = D.reviews[_selectedReview];
    if (!r || r.status !== "en_cours")
        return;
    var entry = (r.entries || [])[parseInt(idx)];
    if (!entry)
        return;
...
function _applyDecision(r, entry, idx, decision) {
...
function _decideThenDerogate(r, entry, idx) {
    entry.decision = "non_conforme";
    entry.decided_at = _today();
    entry.decided_by = (window._currentUser && window._currentUser.name) || "";
    var pid = window.getActiveProjectId ? getActiveProjectId() : null;
    var saved = (pid && window.AccessAPI)
...
function _openNonConformeMeasureModal(r, entry, idx) {
    var app = _findApp(r.application_id);
    ...
    ct_measure_modal.open({ id: "", title: prefilledTitle, description: "", statut: "a_faire", responsable: "", echeance: "" }, {
        ...
        if (!result)
            return; // cancel → keep decision pending
        if (result.__derogation) {
            _decideThenDerogate(r, entry, idx);
            return;
        }
        ...
        entry.decided_by = (window._currentUser && window._currentUser.name) || "";
        var pid = window.getActiveProjectId ? getActiveProjectId() : null;
        var patchPromise = (pid && window.AccessAPI)
            ? AccessAPI.patchEntry(pid, r.id, entry.id, {
        ...
        patchPromise.then(function () {
            if (pid && window.AccessAPI && AccessAPI.createMeasure) {
...
function importReviewCsv() {
    ...
    if (!el)
        return;
    el.value = "";
    el.onchange = function () { if (el.files && el.files[0])
        _doImportCsv(el.files[0]); };
    el.click();
}
...
function _onConnectorImportResult(result) {
...
function _reviewConnector(applicationId) {
...
window.importReviewFromConnector = function () {
    ...
    if (pc && pc.accepts_file) {
        _importReviewFromConnectorFile(pid, r.id, pc);
        return;
    }
    _ctConfirm(t("review.import_connector_confirm_title") || "Importer depuis le connecteur", t("review.import_connector_confirm_body") || "La synchronisation peut prendre quelques secondes. Continuer ?", function () {
...
function _importReviewFromConnectorFile(pid, rid, pc) {
    var el = document.getElementById("connector-file-input");
    if (!el)
        return;
    el.value = "";
    el.onchange = function () {
        ...
        if (file.size > 5 * 1024 * 1024) {
            showStatus(t("plg.file_too_large"), true);
            return;
        }
        _importReviewInFlight = true;
        showStatus(t("review.import_connector_running") || "Synchronisation en cours...");
...
function closeReview() {
    ...
    if (pending.length) {
        showStatus(t("review.pending_remain", { count: pending.length }), true);
        return;
    }
...
function _isRotationOverdue(sa) {
    ...
    if (!sa.last_rotation)
        return true;
    try {
        return (Date.now() - new Date(sa.last_rotation).getTime()) / 86400000 > days;
    }
    catch (e) {
        return false;
    }
...
function _riskLabel(r) { return t("svc.risk." + r) || r; }
function _platformLabel(p) { return t("svc.platform." + p) || p; }
...
function openSA(i) { _selectedSA = parseInt(i); renderPanel(); }
window.openSA = openSA;
...
function addServiceAccount() {
    ...
    if (pid && window.AccessAPI) {
        AccessAPI.createServiceAccount(pid, sa).then(function (created) {
            ...
            _selectedSA = D.service_accounts.length - 1;
            renderPanel();
        }).catch(function (e) { showStatus(e.message, true); });
    }
    else {
...
function deleteServiceAccount(idx0) {
    var idx = parseInt(idx0);
    var sa = (D.service_accounts || [])[idx];
    if (!sa)
        return;
    _ctConfirm(t("svc.confirm_delete", { name: sa.name }), "", function () {
        ...
        if (pid && window.AccessAPI) {
            AccessAPI.deleteServiceAccount(pid, sa.id).then(function () {
                D.service_accounts.splice(idx, 1);
                _selectedSA = null;
                renderPanel();
            }).catch(function (e) { showStatus(e.message, true); });
        }
...
function renderSADetail() {
    var sa = (D.service_accounts || [])[_selectedSA];
    if (!sa)
        return renderSAList();
    var h = '<div class="ct-row ct-row-wrap ct-mb-3">';
...
function saveSAField(field, val) {
    if (_selectedSA === null)
        return;
    var sa = D.service_accounts[_selectedSA];
    if (!sa)
...
// MEASURES
// ═══════════════════════════════════════════════════════════════
function _accessMeasureStatusBadge(statut) {
    ...
    h += ct_table.render({
        ...
        columns: [
            ...
            { key: "statut", label: t("measure.statut"), width: "110px",
                render: function (m) { return _accessMeasureStatusBadge(m.statut); } },
...
window._refreshMeasures = function () {
    var pid = window.getActiveProjectId ? getActiveProjectId() : null;
    if (pid && window.AccessAPI && AccessAPI.listMeasures) {
...
window._editAccessMeasureRow = function (row) {
    ...
    ct_measure_modal.open(m, {
        ...
        onAddNote: function (_entry, fullLog) {
            ...
            var pid = window.getActiveProjectId ? getActiveProjectId() : null;
            if (pid && window.AccessAPI && AccessAPI.patchMeasure)
                return AccessAPI.patchMeasure(pid, m.id, { progress_log: fullLog });
        },
        onDelete: function () {
            ct_modal.confirm({
        ...
        var patch = {};
        allowed.forEach(function (k) { if (result[k] !== undefined)
            patch[k] = result[k]; });
...
// PLUGINS (CONNECTORS)
// ═══════════════════════════════════════════════════════════════
var _pluginList = [], _availablePlugins = [], _pluginsLoaded = false;
function _loadPlugins(cb) {
    var pid = window.getActiveProjectId ? getActiveProjectId() : null;
    if (!pid || !window.AccessAPI) {
        if (cb)
            cb();
    ...
    Promise.all([
        ...
        if (cb)
            cb();
    }).catch(function (e) { console.error("Load plugins:", e); if (cb)
        cb(); });
}
function renderPlugins() {
    if (!_pluginsLoaded) {
        _loadPlugins(function () { renderPanel(); });
        return '<div class="ct-p-5 ct-muted">...</div>';
    }
    var h = '<div class="ct-row ct-row-between ct-mb-3">';
    h += '<h2>' + t("plg.title") + '</h2>';
...
function showPluginModal(pluginId) {
    var pid = window.getActiveProjectId ? getActiveProjectId() : null;
    if (!pid)
        return;
    ...
    if (!existing) {
        ...
        if (!_availablePlugins.length)
            h += '<div class="ct-text-label ct-muted">' + t("plg.no_plugins_available") + '</div>';
    }
    // Label
...
function _renderConfigFields(schema, values) {
...
window._plgSearchFilter = function (query) {
    ...
    if (!grid)
        return;
    var q = (query || "").toLowerCase();
    var cards = grid.querySelectorAll(".plg-type-card");
...
window._plgSelectType = function (type) {
    var hidden = document.getElementById("plg-type");
    if (hidden)
        hidden.value = type;
    var grid = document.getElementById("plg-type-grid");
    if (grid) {
        grid.querySelectorAll(".plg-type-card").forEach(function (c) {
...
window._plgTypeChanged = function () {
    ...
    if (guideWrap)
        guideWrap.innerHTML = ap ? _renderPluginGuide(ap) : "";
};
function _renderPluginGuide(ap) {
...
window._plgTestInline = function (pluginId) {
    ...
    var pluginType = (document.getElementById("plg-type") || {}).value || "";
    if (!pluginType && pluginId) {
        var existing = _pluginList.find(function (p) { return p.id === pluginId; });
        if (existing)
            pluginType = existing.plugin_type;
    }
    ...
    }).catch(function (e) { if (span) {
        span.textContent = e.message;
        span.style.color = "var(--ct-critical)";
    } });
};
var _plgSaveInFlight = false;
window._plgSave = function (existingId) {
    if (_plgSaveInFlight)
        return; // guard against double-click / double-dispatch
    var pid = window.getActiveProjectId ? getActiveProjectId() : null;
    if (!pid)
        return;
    var config = {};
    ...
    var filtersRaw = (document.getElementById("plg-filters") || {}).value || "";
    var filters = filtersRaw ? filtersRaw.split(",").map(function (s) { return s.trim(); }).filter(Boolean) : [];
    var body = {
        label: (document.getElementById("plg-label") || {}).value || "",
        config: config,
        group_filters: filters,
        application_id: (document.getElementById("plg-app") || {}).value || "",
        schedule: (document.getElementById("plg-schedule") || {}).value || "manual",
    ...
    if (existingId) {
        ...
        body.plugin_type = (document.getElementById("plg-type") || {}).value || "";
        if (!body.plugin_type) {
            showStatus("Select a connector type", true);
            return;
        }
...
function _save() { if (window._autoSave)
    window._autoSave();
else if (window._debouncedSave)
...
if (typeof window.ct_handleMeasureDeepLink === "function") {
    window.ct_handleMeasureDeepLink({ open: function (mid) {
            var ms = (D && D.measures) || [];
            if (!ms.some(function (m) { return m.id === mid; }))
                return false;
            if (typeof selectPanel === "function")
                selectPanel("measures");
            if (typeof window._editAccessMeasureRow === "function")
                window._editAccessMeasureRow({ id: mid });
            return true;
        } });
}
...
function _entryKey(r, e) { return (getActiveProjectId() || "") + ":" + r.id + ":" + e.id; }
function _entryLabel(r, e) {
    ...
    return {
        ...
        items: {
            options: function () {
                var out = [];
                D.reviews.forEach(function (r) {
                    (r.entries || []).forEach(function (e) { if (e.decision === "non_conforme")
                        out.push({ id: _entryKey(r, e), label: _entryLabel(r, e) }); });
                });
                return out;
            },
            open: function (id) {
        ...
        measures: {
            ...
            create: function (draft) {
                return ct_measure_modal.open({ title: draft.title, description: draft.description }, {
                    ...
                    }).catch(function (e) { showStatus(e.message || String(e), true); return null; });
                });
            },
            open: function (id) { if (window._editAccessMeasureRow)
                window._editAccessMeasureRow({ id: id.split(":").slice(1).join(":") }); },
        },
