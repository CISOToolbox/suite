...
/** Alerte calculée sur une entrée de revue (admin / ancien / orphelin…). */
type AccessEntryWarning = { code: string; label: string; title: string };

/** Ligne enrichie du tableau Applications (noms réviseurs + dernière revue). */
...
function _compTag(ok?: boolean): string { return '<span class="compliance-tag ' + (ok ? "ok" : "ko") + '">' + (ok ? "✓" : "✗") + '</span>'; }
...
function renderUserList(): string {
    ...
    if (!D.si_users.length) { h += '<div class="empty-state">' + t("user.empty") + '</div>'; return h; }
    ...
    if (!filtered.length) { h += '<div class="empty-state">' + (t("user.no_results") || "Aucun résultat") + '</div>'; return h; }
...
function renderAppList(): string {
    ...
    h += '</div></div>';
    if (!D.applications.length) { h += '<div class="empty-state">' + t("app.empty") + '</div>'; return h; }
    ...
    if (!rows.length) { h += '<div class="empty-state">' + (t("app.no_results") || "Aucune application trouvée") + '</div>'; return h; }
...
window.deleteReviewFromApp = function(reviewId) {
    ...
    if (!r) return;
    if (r.status !== "en_cours") { showStatus("Impossible de supprimer une revue cloturee", true); return; }
    if (!confirm("Supprimer la revue " + reviewId + " ? Toutes les decisions seront perdues.")) return;
    var pid = (window as Window).getActiveProjectId ? getActiveProjectId() : null;
...
function renderReviewList(): string {
    ...
    if (!shownAny) h += '<div class="empty-state">' + t("review.empty") + '</div>';
    return h;
}
...
function renderReviewDetail(): string {
    ...
    if (!isClosed) {
        ...
        if (entries.length && decided === entries.length) h += '<button class="ct-btn mt-8 ct-kpi-tone ct-ml-1" data-write data-variant="primary" data-click="closeReview">' + t("review.close") + '</button>';
    }
...
// SERVICE ACCOUNTS
// ═══════════════════════════════════════════════════════════════
var _ROT_DAYS: Record<string, number> = { "30d": 30, "60d": 60, "90d": 90, "180d": 180, "365d": 365 };
...
function renderSAList(): string {
    ...
    if (!(D.service_accounts || []).length) { h += '<div class="empty-state">' + t("svc.empty") + '</div>'; return h; }
...
function addServiceAccount(): void {
    var sa: AccessServiceAccount = { id: _genId("SVC-", D.service_accounts || []), name: "", identifier: "", platform: "", application_id: "", purpose: "", secret_storage: "unknown", rotation_policy: "unknown", last_rotation: "", owners: [], risk_level: "medium", notes: "" };
...
function _saField(name: string, label: string, type: string, val: unknown): string { return '<div class="form-row"><label>' + esc(label) + '</label><input type="' + type + '" value="' + esc(String(val != null ? val : "")) + '" data-change="saveSAField" data-args=\'["' + name + '"]\' data-pass-value></div>'; }
function _saSel(name: string, label: string, opts: { v: string; l: string }[], val: string | undefined): string { var h = '<div class="form-row"><label>' + esc(label) + '</label><select data-change="saveSAField" data-args=\'["' + name + '"]\' data-pass-value>'; opts.forEach(function(o) { h += '<option value="' + esc(String(o.v)) + '"' + (String(val) === String(o.v) ? " selected" : "") + '>' + esc(o.l) + '</option>'; }); return h + '</select></div>'; }
...
function renderMeasureList(): string {
    ...
    h += '</div></div>';
    if (!D.measures.length) { h += '<div class="empty-state">' + t("measure.empty") + '</div>'; return h; }
...
function renderPlugins(): string {
    ...
    if (!_pluginList.length) { h += '<div class="empty-state">' + t("plg.empty") + '</div>'; return h; }
    ...
    _pluginList.forEach(function(p) {
        ...
        if (p.last_sync_status) h += ' <span class="compliance-tag ' + statusCls + '">' + esc(t("plg.status." + p.last_sync_status) || p.last_sync_status) + '</span>';
        h += '</div>';
...
function showPluginModal(pluginId?: string): void {
    ...
    var panel = ov.querySelector<HTMLElement>(".pwd-panel")!;
    panel.innerHTML = h;
...
window._plgSave = function(existingId) {
    ...
    var okBtn = document.querySelector<HTMLButtonElement>("#confirm-overlay .pwd-ok");
