...
# Access statut (actif/ancien/recrutement) → Pilot statut (actif/inactif).
_ACCESS_TO_PILOT_STATUT = {"actif": "actif", "ancien": "inactif", "recrutement": "actif"}


def siuser_to_pilot_dict(su) -> dict:
    """Map an Access SiUser to Pilot's directory schema."""
    return {
        "email": (su.email or "").strip(),
        "nom": su.nom or "", "prenom": su.prenom or "",
        "fonction": su.fonction or "", "departement": su.equipe or "",
        "statut": _ACCESS_TO_PILOT_STATUT.get(su.statut or "actif", "actif"),
        "manager_email": su.manager_email or "",
    }


async def push_identities_to_pilot(entries: list[dict]) -> None:
    """One-directional self-sync of Access-owned identities to Pilot.

    When an HR connector feeds Access, Access is the source of identities and
    mirrors them into Pilot's directory (Pilot stores them as
    sync_source="access", read-only there, and re-broadcasts to the OTHER
    modules — never back to Access). Fire-and-forget."""
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            await client.post(
                PILOT_URL.rstrip("/") + "/api/internal/directory/from-access",
