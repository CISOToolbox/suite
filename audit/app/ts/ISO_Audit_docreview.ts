...
function renderDocReview(): void {
    ...
    function dkpi(val: string | number, label: string, tone?: string): string {
    ...
    cats.forEach(function(cat) {
        ...
        h += '<h3 style="font-size:0.95em;color:var(--ct-ink);margin:0">' + esc(docs.length ? _rt(docs[0] as unknown as Record<string, any>, "cat") : cat) + '</h3>';
        ...
        docs.forEach(function(d) {
            ...
            if (d.critical) h += ' <span style="color:var(--ct-critical);font-size:0.7em;font-weight:700">★</span>';
            if (d.hds) h += ' <span class="ctrl-hds">HDS</span>';
            h += '</td>';
            // Document name + description
            h += '<td><div style="font-weight:600;font-size:0.85em">' + esc(_rt(d as unknown as Record<string, any>, "label")) + '</div>';
            h += '<div style="font-size:0.78em;color:var(--ct-ink-2)">' + esc(_rt(d as unknown as Record<string, any>, "desc")) + '</div>';
            if (d.linkedControls && d.linkedControls.length) h += '<div style="font-size:0.72em;color:var(--ct-accent);margin-top:2px">§ ' + d.linkedControls.join(', ') + '</div>';
            if (showAlert) h += '<div style="font-size:0.75em;color:var(--ct-critical);margin-top:2px">⚠ ' + esc(_rt(d as unknown as Record<string, any>, "ecartAuto")) + '</div>';
            h += '</td>';
