...
function _openDB(cb: (db: IDBDatabase | null) => void): void {
...
function _imgSave(ctrlId: string, dataUrl: string, name: string, cb?: (id: string | null) => void): void {
    _openDB(function(db) {
        if (!db) { if (cb) cb(null); return; }
        var id = "img_" + Date.now() + "_" + Math.random().toString(36).substr(2, 5);
        var entry: AuditImageEntry = { id: id, ctrlId: ctrlId, data: dataUrl, name: name || "image.jpg", ts: new Date().toISOString() };
...
function _imgGet(imgId: string, cb: (entry: AuditImageEntry | null) => void): void {
    _openDB(function(db) {
        if (!db) { cb(null); return; }
        var req = db.transaction(STORE_NAME, "readonly").objectStore(STORE_NAME).get(imgId);
        req.onsuccess = function() { cb((req.result as AuditImageEntry | undefined) || null); };
        req.onerror = function() { cb(null); };
    });
}
window._imgGet = _imgGet;

function _imgGetAll(ctrlId: string, cb: (imgs: AuditImageEntry[]) => void): void {
    var f = getFinding(ctrlId);
    var ids = f.images || [];
    if (ids.length === 0) { cb([]); return; }
    _openDB(function(db) {
        if (!db) { cb([]); return; }
        var results: AuditImageEntry[] = [];
        var pending = ids.length;
        ids.forEach(function(id) {
            var req = db.transaction(STORE_NAME, "readonly").objectStore(STORE_NAME).get(id);
            req.onsuccess = function() {
                if (req.result) results.push(req.result as AuditImageEntry);
...
function _imgDelete(imgId: string, ctrlId: string, cb?: () => void): void {
    _openDB(function(db) {
...
function _imgCompress(file: File, maxW: number, quality: number, cb: (dataUrl: string) => void): void {
...
function addImage(ctrlId: string): void {
...
function deleteImage(ctrlId: string, imgId: string): void {
    _saveState();
...
function viewImage(imgId: string): void {
    _imgGet(imgId, function(entry) {
        ...
        if (!overlay) {
            ...
            overlay.onclick = function(e) { if (e.target === overlay || (e.target as HTMLElement).className === "img-overlay-content") overlay!.classList.remove("open"); };
            document.body.appendChild(overlay);
        }
        (document.getElementById("image-overlay-img") as HTMLImageElement).src = entry.data;
...
function renderImages(ctrlId: string): void {
...
function _imgExportAll(cb: (images: AuditImageEntry[]) => void): void {
    _openDB(function(db) {
        if (!db) { cb([]); return; }
        var tx = db.transaction(STORE_NAME, "readonly");
        var req = tx.objectStore(STORE_NAME).getAll();
        req.onsuccess = function() { cb((req.result as AuditImageEntry[]) || []); };
        req.onerror = function() { cb([]); };
    });
}

function _imgImportAll(images: AuditImageEntry[], cb?: () => void): void {
    _openDB(function(db) {
...
if (typeof _origSerialize === "function") {
    window._serializeForSave = async function(): Promise<Blob> {
...
if (typeof _origInitDataAndRender === "function") {
    window._initDataAndRender = function(afterFn?: () => void) {
