...
function exportCSV(): void {
    ...
    DOMAINS.forEach(function(d) { domainMap[d.id] = _rt(d as unknown as Record<string, any>, "label"); });
    ...
    CONTROLS.forEach(function(c) {
        ...
        rows.push([
            c.id,
            domainMap[c.d] || c.d,
            _rt(c as unknown as Record<string, any>, "t"),
            lbl,
            f.preuve,
            f.constats,
            f.ecart_critere,
...
function _buildWordDoc(CONTROLS: AuditControl[], DOMAINS: AuditDomain[], imageMap: Record<string, AuditImageEntry[]>): void {
    ...
    DOMAINS.forEach(function(dom) {
        ...
        body += heading(_rt(dom as unknown as Record<string, any>, "label"), 1);
        body += hrLine();
        body += pEmpty();
...
function exportDocReviewCSV(): void {
    ...
    DOC_REVIEW.forEach(function(d) {
        ...
        rows.push([
            d.ref,
            _rt(d as unknown as Record<string, any>, "label"),
